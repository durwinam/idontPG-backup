...
class C:
    ...
    GRN   = "\033[38;2;70;220;130m"   # green      — success / Web Panel
...
def _server_ip_for_web():
    """Best-effort server address for the local Web Panel notice."""
    try:
...
def print_header(title=""):
    ...
    sub = C.R1 + C.BOLD + f"B A C K U P   U T I L I T Y   v {VERSION}   -   {AUTHOR}" + C.RESET
    print(center(sub, 57))
    print()
    print(hline())
    print()
    web_ip = _server_ip_for_web()
    web_url = f"http://{web_ip}:5000"
    print(center(C.GRN + C.BOLD + "✓ WEB PANEL INSTALLED SUCCESSFULLY" + C.RESET, 57))
    print(center(C.GRN + "  Access the Web Panel from your browser:" + C.RESET, 57))
    print()
    print(center(C.GRN + C.BOLD + f"→ {web_url}" + C.RESET, 57))
    print()
    print(hline())
    if title:
        print()
...
UPDATE_URL_BASE   = "https://raw.githubusercontent.com/durwinam/idontPG-backup/main"
...
def workflow_update():
    ...
    print_warning("This downloads the official installer/updater from GitHub (durwinam/idontPG-backup),")
...
def run_set_from_cli():
    """Open the Telegram backup configuration directly from the shell.

    Usage: idont-backup --set
           idontPG-backup --set
    """
    try:
        workflow_backup_bot()
