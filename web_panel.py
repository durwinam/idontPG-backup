...
def _record_login(ip, username, role="user"):
    try:
        STATE_DIR.mkdir(parents=True, exist_ok=True)
        now=time.time(); items=[]
        ...
        items.insert(0,{"time":now,"ip":str(ip or "unknown"),"username":str(username or "admin"),"role":role})
    ...
    def do_POST(self):
        ...
        if path == "/login":
            ...
            if username_ok and check_password(data.get("password", ""), c):
                ...
                login_time=_record_login(login_ip,canonical_username(c.get("username","admin")),"user")
                c["last_login"]={"time":login_time,"ip":login_ip}; save_cfg(c)
                ...
                SESSIONS[sid] = {"created": time.time(), "csrf": secrets.token_urlsafe(24), "role": "user", "username": canonical_username(c.get("username","admin")), "login_notice":{"text":"ورود با موفقیت انجام شد","ip":login_ip}}
                self.send_response(302)
        ...
        if path == ADMIN_PATH:
            if not self.admin_logged():
                ...
                ADMIN_LOGIN_ATTEMPTS.pop(ip,None); login_time=_record_login(ip,canonical_username(c.get("username","admin")),"admin"); c["last_login"]={"time":login_time,"ip":ip}; save_cfg(c); _record_activity(f"ورود مدیر از {ip}","ok"); _notify_login_telegram(c,ip,canonical_username(c.get("username","admin"))); sid=secrets.token_urlsafe(32); SESSIONS[sid]={"created":time.time(),"csrf":secrets.token_urlsafe(24),"role":"admin","username":canonical_username(c.get("username","admin")),"login_notice":{"text":"ورود مدیر با موفقیت انجام شد","ip":ip}}
