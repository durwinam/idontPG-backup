...
(function(){
  ...
  function points(values){const n=values.length,w=900,h=300;return values.map((v,i)=>({x:i*(w/(n-1)),y:h-(Math.max(0,Math.min(100,Number(v)||0))/100)*h}))}
  function path(values){
    const pts=points(values),t=.16;
    ...
    for(let i=0;i<pts.length-1;i++){
      ...
      c1y=Math.max(0,Math.min(300,c1y)); c2y=Math.max(0,Math.min(300,c2y));
...
/* v5.6.4 language control: SVG glow, crisp active state */
.language-toggle{position:relative;display:grid;place-items:center;overflow:visible}
.language-svg{width:21px;height:21px;fill:none;stroke:currentColor;stroke-width:1.65;stroke-linecap:round;stroke-linejoin:round;filter:drop-shadow(0 0 5px rgba(34,211,238,.55));transition:transform .25s ease,filter .25s ease}
.language-toggle:hover .language-svg{transform:rotate(8deg) scale(1.08);filter:drop-shadow(0 0 8px rgba(34,211,238,.85))}
.language-toggle:after{content:"";position:absolute;inset:7px;border-radius:50%;background:radial-gradient(circle,rgba(34,211,238,.20),transparent 70%);pointer-events:none}
.language-menu{overflow:hidden}
.language-menu button{direction:ltr;text-align:left}
.language-menu button .lang-mark{width:29px;height:24px;display:grid;place-items:center;border-radius:8px;font-size:9px;font-weight:900;letter-spacing:.4px;color:#67e8f9;background:linear-gradient(145deg,rgba(34,211,238,.13),rgba(139,92,246,.16));border:1px solid rgba(34,211,238,.20);box-shadow:0 0 12px rgba(34,211,238,.10)}
.language-menu button.active .lang-mark{color:#fff;background:linear-gradient(135deg,rgba(34,211,238,.28),rgba(139,92,246,.30));border-color:rgba(34,211,238,.42);box-shadow:0 0 16px rgba(34,211,238,.22)}
body.light .language-svg{filter:drop-shadow(0 0 4px rgba(124,58,237,.32))}
body.light .language-menu button .lang-mark{color:#6d28d9;background:linear-gradient(145deg,rgba(124,58,237,.08),rgba(236,72,153,.08));border-color:rgba(124,58,237,.16)}
...
def ui_language_menu():
    return '''<div class="language-picker">
      <button class="language-toggle" id="languageToggle" type="button" aria-label="Language" title="Language">
        <svg class="language-svg" viewBox="0 0 24 24" aria-hidden="true">
          <circle cx="12" cy="12" r="8.5"></circle>
          <path d="M3.8 12h16.4M12 3.5c2.2 2.35 3.3 5.2 3.3 8.5s-1.1 6.15-3.3 8.5c-2.2-2.35-3.3-5.2-3.3-8.5S9.8 5.85 12 3.5z"></path>
          <path d="M5.1 7.2c2 1 4.3 1.5 6.9 1.5s4.9-.5 6.9-1.5M5.1 16.8c2-1 4.3-1.5 6.9-1.5s4.9.5-6.9 1.5"></path>
        </svg>
      </button>
      <div class="language-menu" id="languageMenu" hidden>
        <button type="button" data-lang-choice="en"><span class="lang-mark">EN</span><span>English</span></button>
        <button type="button" data-lang-choice="fa"><span class="lang-mark">فا</span><span>فارسی</span></button>
        <button type="button" data-lang-choice="ru"><span class="lang-mark">РУ</span><span>Русский</span></button>
      </div>
    </div>'''

def ui_language_script():
    return r'''(function(){
const LANG_KEY="idontpg-lang-v3";
const maps={fa:{},en:{"پاسارگارد":"PasarGuard","داشبورد":"Dashboard","کلی":"Overall","داشبورد مدیریت پاسارگارد":"PasarGuard Management Dashboard","پیش‌فرض":"Default","نامحدود":"Unlimited","فعال":"Active","آنلاین":"Online","پلتفرم":"Platform","همه وضعیت‌ها":"All Statuses","وضعیت":"Status","آمار":"Statistics","انتخاب همه":"Select All","لغو انتخاب همه":"Deselect All","در حال ویرایش...":"Modifying...","در حال حذف...":"Removing...","در حال ایجاد...":"Creating","هاست‌ها":"Hosts","گروه‌ها":"Groups","مستندات":"Documentation","گروه گفتگو":"Discussion Group","GitHub":"GitHub","جامعه":"Community","پشتیبانی از ما":"Support Us","مدیریت و کنترل هاست‌ها":"Manage and control hosts","مدیریت و کنترل تنظیمات":"Manage and control settings","مدیریت کاربران":"Monitor Users","همه زمان":"All time","امروز":"Today","کپی شناسه":"Copy ID","در حال ذخیره...":"Saving...","عمومی":"General","در حال بارگذاری...":"Loading...","پاک کردن":"Clear","هرگز":"Never","منابع":"resources","هسته‌ها":"Cores","تایید":"Confirm","لغو":"Cancel","ایجاد شده":"Created","توسط":"by","آی‌پی":"IP","مدیر":"Admin","sudo":"Sudo","ذخیره":"Save","تست":"Test","در حال تست":"Testing","اقدامات":"Actions","جزئیات":"Details","زمان ویرایش":"Edited at","آیا مطمئن هستید که می‌خواهید مصرف کاربر ادمین <b>{{name}}</b> را ریست کنید؟":"Are you sure you want to reset the <b>{{name}}</b> admin users usage?","آیا مطمئن هستید که می‌خواهید همه کاربران تحت مدیریت ادمین <b>{{name}}</b> را حذف کنید؟ این عمل قابل بازگشت نیست.":"Are you sure you want to remove all users under admin <b>{{name}}</b>? This action cannot be undone.","غیرفعال‌سازی مدیر":"Disable Admin","فعال‌سازی مدیر":"Enable Admin","آیا مطمئن هستید که می‌خواهید مدیر <b>{{name}}</b> را غیرفعال کنید؟":"Are you sure you want to disable admin <b>{{name}}</b>?","آیا مطمئن هستید که می‌خواهید مدیر <b>{{name}}</b> را فعال کنید؟":"Are you sure you want to enable admin <b>{{name}}</b>?","آیا مطمئن هستید که می‌خواهید مدیر <b>{{name}}</b> را حذف کنید؟":"Are you sure you want to remove the <b>{{name}}</b> admin?","آیا می‌خواهید همه کاربران تحت مدیریت {{name}} را فعال کنید؟":"Do you want to activate all users under {{name}}?","آیا می‌خواهید همه کاربران تحت مدیریت {{name}} را غیرفعال کنید؟":"Do you want to disable all users under {{name}}?","کنترل، بروزرسانی، و ترتیب حساب‌های کاربری":"Control, Update, and Arrange User Accounts","کنترل، بروزرسانی، و ترتیب گروه‌های کاربری":"Control, Update, and Arrange User Groups","نظارت، کنترل، افزودن، ویرایش و حذف گره‌ها":"Monitor, control, create, modify, remove nodes","ویرایش":"Modify","ویرایش گروه":"Modify Group","غیرفعال کردن":"Disable","هشدار":"Warning","حذف":"Delete","قالب‌های کاربر":"User Templates","قالب‌های کلاینت":"Client Templates","ویرایش قالب کاربر":"Modify User Template","ایجاد قالب کاربر":"Create User Template","پیکربندی":"Configuration","مشکلی پیش آمده، لطفا پیکربندی را بررسی کنید":"Something went wrong, please check the configuration","گزارش":"Logs","اجرا مجدد هسته":"Restart Core","درحال اجرا مجدد...":"Restarting...","بسته شده":"Closed","متصل":"Connected","در حال اتصال...":"Connecting...","عدم اتصال":"Not Connected","تنظیمات هسته با موفقیت ثبت شد":"Core settings updated successfully","تنظیمات هسته":"Core Configs Settings","ساخت کاربر":"Create new user","افزودن کاربر":"Create User","افزودن گروه":"Create Group","مصرف داده":"data usage","MM/dd/yyyy":"MMMM d, yyy","گره {{name}} با موفقیت حذف شد":"Node {{name}} removed successfully","از حذف گره <b>{{name}}</b> مطمئن هستید؟":"Are you sure you want to remove the {{name}} node?","حذف گره":"Remove Node","حذف هاست":"Remove Host","آیا مطمئن هستید که می‌خواهید هاست <b>{{name}}</b> را حذف کنید؟":"Are you sure you want to remove the {{name}} host?","هاست {{name}} با موفقیت حذف شد":"Host {{name}} removed successfully","حذف هاست {{name}} ناموفق بود":"Failed to remove host {{name}}","{{count}} هاست با موفقیت حذف شد.":"{{count}} hosts deleted successfully.","حذف هاست‌های انتخاب‌شده ناموفق بود.":"Failed to delete selected hosts.","حذف هاست‌های انتخاب‌شده":"Delete Selected Hosts","آیا مطمئن هستید که می‌خواهید {{count}} هاست انتخاب‌شده را حذف کنید؟ این عمل قابل بازگشت نیست.":"Are you sure you want to delete {{count}} selected hosts? This action cannot be undone.","گره {{name}} با موفقیت بروزرسانی شد":"Node {{name}} has been modified successfully","ویرایش گره":"Modify Node","{{username}} با موفقیت حذف شد.":"{{username}} removed successfully.","از حذف کاربر <b>{{username}}</b> مطمئن هستید?":"Are you sure you want to remove {{username}}?","حذف کاربر":"Delete User","غیر فعال":"disabled","انقضا":"Expire","تاریخ انقضا":"Expire Date","آخرین آنلاین":"Last Online","{{time}} منقضی شده":"Expired {{time}}","پایان در {{time}}":"Expires in {{time}}","گزینه‌های مرتب‌سازی":"Sort Options","کمک مالی":"Donation","تنظیمات هاست":"Host Settings","خروج":"Logout","تنظیمات گره‌ها":"Node Settings","نمودار گره‌ها":"Nodes Usage","ورودی":"Inbound","ورودی‌ها":"Inbounds","آدرس":"Address","مصرف":"Usage","حالت نمایش":"View mode","نمای شبکه‌ای":"Grid view","نمای لیستی":"List view","اسم هاست":"Host Name","ایجاد":"Create","تعداد در صفحه":"Items per page","ورود":"Login","این فیلد باید پر شود!":"This field is required","وارد حساب خود شوید":"Login to your account","خوش آمدید, لطفا اطلاعات خود را وارد کنید":"Welcome back, please enter your details","بازگشت به ورود":"Back to login","مصرف حافظه":"memory usage","بعدی":"Next","نظارت بر سرورها و کاربران شما":"Monitor your servers and users","کنترل جزئیات سرور شما":"Control your server details","استفاده از این گره به عنوان هاست تمام ورودی‌ها":"Add this node as a new host for every inbound","افزودن گره پاسارگارد":"Create New PasarGuard Node","گره {{name}} با موفقیت افزوده شد":"Node {{name}} created successfully","گواهی":"Certificate","پورت API":"API Port","نام":"Name","پورت":"Port","اتصال مجدد":"Reconnect","درحال اتصال مجدد...":"Reconnecting...","گره‌ها":"Using PasarGuard-Node, you are able to scale up your connection quality by creating different nodes on different servers.","ضریب گره":"Usage Ratio","در انتظار اتصال":"On Hold","تکرار":"Duplicate","ایجاد و مدیریت قالب‌ها":"Create and manage templates","گذرواژه":"Password","قبلی":"Previous","بازگشت":"Back","کد QR":"QR Code","لینک اشتراک {{username}}":"{{username}}'s Subscribe Link","اشتراک {{username}}":"{{username}}'s Subscription","لینک اشتراک":"Subscription Link","کانفیگ‌ها":"Configurations","کپی همه":"Copy All","کپی کانفیگ":"Copy Config","دریافت کانفیگ‌ها ناموفق بود":"Failed to fetch configurations","کانفیگی یافت نشد":"No configurations found","کانفیگ ناشناخته":"Unknown Config","فرمت کد QR":"QR Code Format","کانفیگ":"Config","کپی Base64":"Copy Base64","دانلود وایرگارد":"Download WireGuard","بازنشانی":"Revoke","بازنشانی کل مصرف":"Reset All Usages","بازنشانی مصرف انجام نشد، دوباره تلاش کنید.":"Usage reset failed, please try again.","این عمل مصرف تمام کاربران را بازنشانی می‌کند. از این بازنشانی مطمئن هستید؟ این عمل قابل بازگشت نیست!":"This action clears all of the users' data usage completely. Are you sure you want to reset all usage? THIS CANNOT BE UNDONE!","مصرف کل با موفقیت بازنشانی شد.":"All usage has reset successfully.","بازنشانی مصرف تمام کاربران":"Reset data usage for all users","از بازنشانی مصرف <b>{{username}}</b> مطئمن هستید؟":"Are you sure you want to reset <b>{{username}}</b>'s usage?","مصرف {{username}} با موفقیت بازنشانی شد.":"{{username}}'s usage has reset successfully.","بازنشانی مصرف کاربر":"Reset User Usage","بازنشانی اشتراک انجام نشد، لطفاً دوباره امتحان کنید.":"Subscription revoke failed, please try again.","آیا مطمئن هستید که می خواهید اشتراک «{{username}}» را بازنشانی کنید؟":"Are you sure you want to revoke «{{username}}»'s subscription?","اشتراک {{username}} با موفقیت بازنشانی شد.":"{{username}}'s subscription has revoked successfully.","بازنشانی اشتراک کاربر":"Revoke User Subscription","جستجو":"Search","غیرفعال":"Disabled","منقضی شده":"Expired","اتمام حد مصرف":"Limited","خطا در اتصال":"Error","درحال اتصال":"Connecting","نام کاربری":"Username","کاربران":"Users","کپی شد":"Copied","تنظیمات کپی":"Copy Configs","کپی لینک اشتراک":"Copy Subscription Link","کد QR اشتراک":"Sub QR Code","کد QR تنظیمات":"Configs QR Code","کاربری افزوده نشده است":"There is no user Created to the system","به‌نظر میرسه کاربری که جستجو کردید، وجود ندارد":"It seems there is no user matched with what you are looking for","هیچ کاربری پیکربندی نشده":"No users configured","با ایجاد اولین حساب کاربری شروع کنید.":"Get started by creating your first user account.","هیچ کاربری با معیارهای جستجوی شما مطابقت ندارد. لطفاً عبارات جستجوی خود را تغییر دهید.":"No users match your search criteria. Try adjusting your search terms.","مجموع":"Total","سیستم":"System","کل ترافیک":"Total Traffic","مصرف رم":"RAM Usage","مصرف پردازنده":"CPU Usage","مصرف دیسک":"Disk Usage","زمان روشن بودن":"Uptime","هسته":"cores","داده‌های لحظه‌ای":"Real-Time Data","داده‌های تاریخی":"Historical Data","مشاهده تاریخی":"View Historical","مشاهده لحظه‌ای":"View Realtime","تاریخی":"Historical","لحظه‌ای":"Realtime","آمار و نمودارهای زنده برای سرورها و کاربران شما.":"Live statistics and charts for your servers and users.","داده‌های عملکرد تاریخی سرور برای بازه زمانی انتخاب شده":"Historical server performance data for the selected time range","اشتراک‌ها":"Subscriptions","توزیع کلاینت‌های اشتراک":"Subscription Client Distribution","جزئیات کلاینت‌های اشتراک":"Subscription Client Details","سهم به‌روزرسانی‌های لینک اشتراک بین عامل‌ها و برنامه‌ها.":"Share of subscription updates across agents and applications.","به‌طور پیش‌فرض فقط {{limit}} به‌روزرسانی اخیر اشتراک برای هر کاربر نگه داشته می‌شود. برای تغییر، {{env}} را در محیط تنظیم کنید.":"By default only the latest {{limit}} subscription updates are kept per user. Change this with {{env}} in your environment.","مجموع به‌روزرسانی‌ها":"Total subscription updates","درصد":"Percentage","فیلتر بر اساس ادمین":"Filter by admin","یک ادمین را انتخاب کنید":"Choose an admin","همه ادمین‌ها":"All admins","{{client}} با سهم {{percentage}}٪ پیشتاز به‌روزرسانی‌ها است":"{{client}} leads with {{percentage}}% of subscription updates","به‌روزرسانی در دوره زمانی":"Updates During Period","بازه":"Period","این نمودار {{count}} شمع دارد. نمودار ناحیه‌ای از تنظیمات تم معمولاً برای بازه‌های بزرگ روان‌تر است.":"This chart has {{count}} candles. Area charts from Theme settings usually perform better with large ranges.","استفاده از نمودار ناحیه‌ای":"Use area chart","خودکار":"Auto","انتخاب بازه زمانی":"Select Time Range","دوره مورد نظر را برای تجزیه و تحلیل داده‌های تاریخی انتخاب کنید":"Choose a date range above to view traffic usage statistics.","لطفاً یک بازه زمانی برای مشاهده داده‌های تاریخی انتخاب کنید":"Please select a time range to view historical data","فعالسازی":"Enable","ویرایش هاست":"Modify Host","موفق":"Success","خطا":"Error","مشاهده آموزش":"View tutorial","مدیریت هسته‌های شما":"Manage Your Cores","افزودن هسته":"Create Core","هیچ هسته‌ای موجود نیست. یکی اضافه کنید تا شروع کنید.":"No cores available. Create one to get started.","پیکربندی هسته «{{name}}» با موفقیت کپی شد":"Core config «{{name}}» duplicated successfully","خطا در کپی پیکربندی هسته «{{name}}»":"Failed to duplicate core config «{{name}}»","هسته با موفقیت حذف شد":"Core has been removed successfully","حذف هسته با خطا مواجه شد":"Failed to remove core","حذف هسته":"Remove Core Config","هسته یافت نشد":"Core not found","تاریخ ایجاد":"Created at","تغییر وضعیت":"Toggle status","هسته «{{name}}» با موفقیت تغییر وضعیت داده شد":"Core «{name}» has been toggled successfully","تغییر وضعیت هسته «{{name}}» با خطا مواجه شد":"Failed to toggle core «{name}»","هسته «{{name}}» با موفقیت حذف شد":"Core «{name}» has been removed successfully","حذف هسته «{{name}}» با خطا مواجه شد":"Failed to remove core «{name}»","آیا مطمئن هستید که می‌خواهید هسته «{{name}}» را حذف کنید؟":"Are you sure you want to remove {{name}}?","{{count}} هسته با موفقیت حذف شد.":"{{count}} cores deleted successfully.","حذف هسته‌های انتخاب‌شده ناموفق بود.":"Failed to delete selected cores.","حذف هسته‌های انتخاب‌شده":"Delete Selected Cores","آیا مطمئن هستید که می‌خواهید {{count}} هسته انتخاب‌شده را حذف کنید؟ این عمل قابل بازگشت نیست.":"Are you sure you want to delete {{count}} selected cores? This action cannot be undone.","بستن":"Close","مشاهده":"View","کپی":"Copy","کپی شد!":"Copied!","کلیک برای کپی":"Click to copy","کپی ناموفق بود":"Failed to copy","دانلود کانفیگ ناموفق بود":"Failed to download config","بیشتر":"more","باز کردن منو":"Expand sidebar","بستن منو":"Collapse sidebar","آیا مطمئن هستید که می‌خواهید کاربر «{{name}}» را حذف کنید؟ این عملیات غیرقابل بازگشت است.":"Are you sure you want to delete user «{{name}}»? This action cannot be undone.","کاربر «{{name}}» با موفقیت حذف شد.":"User «{{name}}» has been deleted successfully.","حذف کاربر «{{name}}» با خطا مواجه شد. {{error}}":"Failed to delete user «{{name}}». {{error}}","میزان مصرف ترافیک":"Traffic Usage","کل مصرف ترافیک در تمام سرورها":"Total traffic usage across all servers","مصرف در دوره زمانی":"Usage During Period","کاربران آنلاین":"Online Users","تعداد کاربران":"User Count","تعداد کاربران آنلاین، منقضی‌شده و محدودشده در طول زمان":"Online, expired, and limited user activity counts over time","تغییر وضعیت می‌تواند تاریخچه را نادقیق کند. بازنشانی مصرف فقط با فعال بودن پاک‌سازی داده‌های نمودار تاریخچه را حذف می‌کند.":"Status changes can skew history. Usage resets clear chart history only if chart-data cleanup is enabled.","گروه‌بندی بر اساس گره":"Group by node","تعداد در بازه":"Count During Period","سلامت پراسس ها":"Workers Health","وضعیت زمان‌بند و پراسس گره":"Scheduler and node worker status","زنده":"Live","نمایش جزئیات":"Toggle details","بارگذاری سلامت پراسس ها ممکن نیست.":"Unable to load worker health.","زمان‌بند":"Scheduler","پراسس گره":"Node Worker","NATS غیرفعال است. بررسی سلامت پراسس ها در دسترس نیست.":"NATS is disabled. Worker health checks are unavailable.","سالم":"Healthy","در حال اجرا":"Running","در دسترس":"Available","کاهش‌یافته":"Degraded","غیرقابل‌دسترسی":"Unavailable","از کار افتاده":"Down","نامشخص":"Unknown","کاربران محدود شده":"Limited Users","کاربران فعال":"Active Users","کاربران غیرفعال":"Disabled Users","کاربران منقضی شده":"Expired Users","کاربران در انتظار":"On Hold Users","همه کاربران":"All Users","ترافیک سرور":"Server Traffic","توزیع ترافیک گره ها":"Node Traffic Distribution","هیچ گره ای در دسترس نیست":"No Nodes Available","هیچ داده‌ای در دسترس نیست":"No Data Available","یک بازه زمانی را انتخاب کنید تا آمار و روند مصرف ترافیک را مشاهده کنید.":"Select a time range to view traffic usage statistics and trends.","هیچ گره ای در حال حاضر متصل نیست تا توزیع ترافیک را نمایش دهد.":"No nodes are currently connected to display traffic distribution.","در حال دریافت آخرین داده‌های آمار...":"Fetching the latest statistics data...","در انتظار داده‌ها":"Waiting for Data","آمار لحظه‌ای در اینجا نمایش داده می‌شود پس از شروع جریان داده‌ها.":"Real-time statistics will appear here once data starts flowing.","داده‌ای در بازه انتخابی وجود ندارد":"No Data in Selected Range","هیچ داده ترافیکی برای دوره زمانی انتخابی یافت نشد. بازه زمانی متفاوتی را انتخاب کنید.":"No traffic data found for the selected time period. Try selecting a different date range.","یک گره را انتخاب کنید تا آمار دقیق را مشاهده کنید":"Select a node to view detailed statistics","نمای کلی از وضعیت و فعالیت حساب‌های کاربری":"Overview of user account status and activity","اتصال با خطا مواجه شد. لطفاً شبکه خود را بررسی کنید و دوباره تلاش کنید.":"Connection failed. Please check your network and try again.","بارگذاری آمار با خطا مواجه شد":"Failed to Load Statistics","آیا مطمئن هستید که می‌خواهید مصرف «{{name}}» را ریست کنید؟":"Are you sure you want to reset the usage for «{{name}}»?","بازنشانی مصرف":"Reset Usage","مصرف کاربر «{{name}}» با موفقیت ریست شد":"User «{{name}}» usage has been reset successfully","بازنشانی مصرف کاربر «{{name}}» ناموفق بود":"Failed to reset usage for user «{{name}}»","هنوز متصل نشده":"Not connected yet","نسخه هسته":"Core Version","نسخه گره":"Node Version","عملیات موفق بود!":"Operation successful!","عملیات ناموفق بود!":"Operation failed!","در حال اعمال...":"Applying...","تنظیمات با موفقیت اعمال شد!":"Settings applied successfully!","تگ‌های ورودی":"Inbound Tags","جستجوی ورودی‌ها...":"Search inbounds...","هیچ ورودی‌ای یافت نشد":"No inbounds found","تاریخ":"Date","مصرف کل":"Total Usage","ارسال":"Uplink","دریافت":"Downlink","آمار گره‌ها":"Node Statistics","آمار دقیق ترافیک هر گره در این بازه زمانی. روی میله‌های نمودار کلیک کنید تا جزئیات بیشتری ببینید.":"Detailed traffic statistics for each node at this time period. Click on bars in the chart to view more details.","برای جزئیات کلیک کنید":"Click for details","برای جزئیات بیشتر کلیک کنید":"Click for more details","انتخاب تاریخ":"Pick a date","پاک کردن همه فیلترها":"Clear All Filters","همه":"All","قوانین وضعیت کاربر":"User Status Rules","هاست‌ها را بر اساس وضعیت‌های کاربرِ پیکربندی‌شده فیلتر می‌کند (فعال، در انتظار، محدود و ...).":"Matches hosts by configured user statuses (active, on-hold, limited, etc).","پاک کردن همه اینباندها":"Clear all inbounds","وضعیت هاست":"Host State","مشخص می‌کند خود هاست فعال است یا غیرفعال.":"Filters whether the host itself is enabled or disabled.","هاست‌های فعال":"Enabled Hosts","هاست‌های غیرفعال":"Disabled Hosts","نمای کلی سیستم":"System overview","بکاپ تلگرام":"Telegram Backup","تنظیمات و ارسال":"Settings & delivery","تنظیمات بکاپ":"Backup Settings","Scheduler و Backup":"Scheduler & Backup","تست تلگرام":"Telegram Test","بررسی اتصال":"Connection check","حساب کاربری":"Account","مدیریت ورود":"Login management","پایان نشست":"End session","منوی اصلی":"Main menu","مدیریت Backupها":"Manage Backups","Backupهای موجود را مشاهده، دانلود یا حذف کنید.":"View, download or delete available backups.","هنوز Backupای پیدا نشد.":"No backups found.","اطلاعات Backup":"Backup Information","تعداد Backup":"Backup count","حجم کل Backupها":"Total backup size","آخرین Backup":"Latest backup","هنوز Backup ساخته نشده":"No backup created yet","قابل دریافت نیست":"Unavailable","لینک پنل":"Panel link","وضعیت پنل":"Panel status","مصرف واقعی Node":"Real Node usage","تنظیم نشده":"Not configured","متوقف":"Stopped","Telegram Backup":"Telegram Backup","Bot Token":"Bot Token","Chat ID":"Chat ID","Topic ID":"Topic ID","Proxy":"Proxy","بدون Proxy":"No Proxy","تنظیمات Telegram":"Telegram Settings","ارسال تست":"Send Test","تنظیمات Backup":"Backup Settings","Backup Controls":"Backup Controls","مدیریت Backup":"Manage Backup","شروع Backup دستی":"Run Manual Backup","ارسال تست پیام به Telegram":"Send a Test Message to Telegram","قبل از فعال‌کردن Scheduler اتصال را بررسی کنید.":"Check the connection before enabling the scheduler.","Backup بعدی":"Next Backup","Next Scheduled Backup":"Next Scheduled Backup","Backup هفت روز اخیر":"Backup — Last 7 Days","Backup Activity":"Backup Activity","فضای دیسک":"Disk Space","Disk Monitor":"Disk Monitor","وضعیت سرویس‌ها":"Service status","Health Check":"Health Check","منابع سرور مجازی":"Virtual Server Resources","CPU · RAM · Disk · پایش لحظه‌ای":"CPU · RAM · Disk · Live monitoring","وضعیت لحظه‌ای سرور":"Live server status","LIVE TELEMETRY":"LIVE TELEMETRY","به‌روزرسانی":"Updated","اکنون":"Now","فعالیت‌های اخیر":"Recent Activity","آخرین فعالیت‌ها":"Recent Activity","Recent Activity":"Recent Activity","تعداد فعالیت‌های اخیر":"Recent activity count","همین الان":"Just now","دقیقه پیش":"minutes ago","ساعت پیش":"hours ago","روز پیش":"days ago","روز":"days","ساعت":"hours","دقیقه":"minutes","مدیریت خصوصی":"Private Administration","این بخش فقط برای مدیر اصلی است.":"This area is for the main administrator only.","ورود به پنل":"Panel Login","برای ورود، نام کاربری و رمز عبور ادمین را وارد کنید.":"Enter the admin username and password to continue.","ورود امن":"Secure Login","راه‌اندازی اولیه":"Initial Setup","ساخت رمز و ورود":"Create Password & Login","رمز مدیر":"Admin Password","تکرار رمز":"Confirm Password","رمز جدید":"New Password","تکرار رمز جدید":"Confirm New Password","تغییر رمز":"Change Password","مرکز مدیریت":"Control Center","شخصی‌سازی کامل ظاهر، داشبورد و اجزای وب‌پنل.":"Customize the complete look, dashboard and web-panel components.","شخصی‌سازی":"Customization","شش تم آماده":"Six Ready Themes","کاربر عادی هیچ گزینه‌ای برای انتخاب تم نمی‌بیند.":"Regular users cannot choose the admin theme.","ظاهر":"Appearance","رنگ اصلی":"Primary color","رنگ دوم":"Secondary color","Background":"Background","Logo URL":"Logo URL","عرض Logo":"Logo width","شدت Glow":"Glow intensity","Glow و حرکت":"Glow & Motion","سرعت انیمیشن":"Animation speed","متن و برند":"Text & Branding","نام پنل":"Panel name","عنوان داشبورد":"Dashboard title","عنوان بکاپ":"Backup title","عنوان حجم مصرفی":"Traffic title","عنوان فعالیت‌های اخیر":"Recent activity title","چیدمان و تجربه کاربر":"Layout & User Experience","عرض محتوای داشبورد":"Dashboard content width","فاصله بین کارت‌ها":"Card spacing","بخش‌های داشبورد":"Dashboard sections","حجم مصرفی":"Traffic usage","آمار سرور":"Server statistics","دکمه سفارشی":"Custom Button","دکمه‌های سفارشی":"Custom Buttons","آیکون":"Icon","لینک":"Link","افزودن":"Add","ذخیره تغییرات":"Save Changes","کاستوم":"Custom","لایت":"Light","دارک":"Dark","بکاپ دستی":"Manual Backup","زمان‌بندی خودکار":"Automatic Scheduler","بازه Backup (ساعت)":"Backup interval (hours)","شامل PG-Node شود":"Include PG-Node","ذخیره و شروع":"Save & Start","توقف":"Stop","همین حالا یک Backup کامل بگیرید.":"Create a full backup now.","یک پیام آزمایشی با تنظیمات فعلی ارسال می‌شود.":"A test message will be sent using the current settings.","ارسال پیام تست":"Send Test Message","پیام تست با موفقیت ارسال شد.":"Test message sent successfully.","ارسال پیام تست ناموفق بود.":"Test message failed.","Scheduler ذخیره و شروع شد.":"Scheduler saved and started.","Scheduler ذخیره شد ولی شروع آن با خطا مواجه شد.":"Scheduler saved, but starting it failed.","تنظیمات عمومی ذخیره شد.":"General settings saved.","تنظیمات Telegram با موفقیت ذخیره شد.":"Telegram settings saved successfully.","تنظیمات حساب با موفقیت ذخیره شد.":"Account settings saved successfully.","تنظیمات با موفقیت ذخیره شد و روی کل وب‌پنل اعمال شد.":"Settings saved and applied across the web panel.","آفلاین":"Offline","تنظیم شده":"Configured","نامعتبر":"Invalid"},ru:{"پاسارگارد":"PasarGuard","داشبورد":"Панель","کلی":"Общее","داشبورد مدیریت پاسارگارد":"Панель управления PasarGuard","پیش‌فرض":"По умолчанию","نامحدود":"Без ограничений","فعال":"Активно","آنلاین":"Онлайн","پلتفرم":"Платформа","همه وضعیت‌ها":"Все статусы","وضعیت":"Статус","آمار":"Статистика","انتخاب همه":"Выбрать все","لغو انتخاب همه":"Снять выделение со всех","در حال ویرایش...":"Изменение...","در حال حذف...":"Удаление...","در حال ایجاد...":"Создание...","هاست‌ها":"Хосты","گروه‌ها":"Группы","مستندات":"Документация","گروه گفتگو":"Группа обсуждений","GitHub":"GitHub","جامعه":"Сообщество","پشتیبانی از ما":"Поддержать нас","مدیریت و کنترل هاست‌ها":"Управление и контроль хостов","مدیریت و کنترل تنظیمات":"Управление и контроль настроек","مدیریت کاربران":"Мониторинг пользователей","همه زمان":"Все время","امروز":"Сегодня","کپی شناسه":"Копировать ID","در حال ذخیره...":"Сохранение...","عمومی":"Общие","در حال بارگذاری...":"Загрузка...","پاک کردن":"Очистить","هرگز":"Никогда","منابع":"ресурсы","هسته‌ها":"Ядра","تایید":"Подтвердить","لغو":"Отмена","ایجاد شده":"Создано","توسط":"от","آی‌پی":"IP","مدیر":"Админ","sudo":"Суперпользователь","ذخیره":"Сохранить","تست":"Тест","در حال تست":"Тестирование","اقدامات":"Действия","جزئیات":"Детали","زمان ویرایش":"Изменено","آیا مطمئن هستید که می‌خواهید مصرف کاربر ادمین <b>{{name}}</b> را ریست کنید؟":"Вы уверены, что хотите сбросить использование пользователя-админа <b>{{name}}</b>?","آیا مطمئن هستید که می‌خواهید همه کاربران تحت مدیریت ادمین <b>{{name}}</b> را حذف کنید؟ این عمل قابل بازگشت نیست.":"Вы уверены, что хотите удалить всех пользователей под управлением администратора <b>{{name}}</b>? Это действие нельзя отменить.","غیرفعال‌سازی مدیر":"Отключить администратора","فعال‌سازی مدیر":"Включить администратора","آیا مطمئن هستید که می‌خواهید مدیر <b>{{name}}</b> را غیرفعال کنید؟":"Вы уверены, что хотите отключить администратора <b>{{name}}</b>?","آیا مطمئن هستید که می‌خواهید مدیر <b>{{name}}</b> را فعال کنید؟":"Вы уверены, что хотите включить администратора <b>{{name}}</b>?","آیا مطمئن هستید که می‌خواهید مدیر <b>{{name}}</b> را حذف کنید؟":"Вы уверены, что хотите удалить администратора <b>{{name}}</b>?","آیا می‌خواهید همه کاربران تحت مدیریت {{name}} را فعال کنید؟":"Вы хотите активировать всех пользователей под управлением {{name}}?","آیا می‌خواهید همه کاربران تحت مدیریت {{name}} را غیرفعال کنید؟":"Вы хотите отключить всех пользователей под управлением {{name}}?","کنترل، بروزرسانی، و ترتیب حساب‌های کاربری":"Контроль, обновление и упорядочивание учетных записей","کنترل، بروزرسانی، و ترتیب گروه‌های کاربری":"Контроль, обновление и упорядочивание групп пользователей","نظارت، کنترل، افزودن، ویرایش و حذف گره‌ها":"Мониторинг, управление, добавление, редактирование, удаление узлов","ویرایش":"Изменить","ویرایش گروه":"Редактировать группу","غیرفعال کردن":"Отключить","هشدار":"Предупреждение","حذف":"Удалить","قالب‌های کاربر":"Шаблоны пользователей","قالب‌های کلاینت":"Шаблоны клиентов","ویرایش قالب کاربر":"Редактировать шаблон пользователя","ایجاد قالب کاربر":"Создать шаблон пользователя","پیکربندی":"Конфигурация","مشکلی پیش آمده، لطفا پیکربندی را بررسی کنید":"Что-то пошло не так, пожалуйста, проверьте конфигурацию","گزارش":"Логи","اجرا مجدد هسته":"Перезагрузить ядро","درحال اجرا مجدد...":"Перезагрузка...","بسته شده":"Закрыто","متصل":"Подключен","در حال اتصال...":"Соединение...","عدم اتصال":"Не подключено","تنظیمات هسته با موفقیت ثبت شد":"Основные настройки успешно обновлены","تنظیمات هسته":"Основные настройки","ساخت کاربر":"Создать нового пользователя","افزودن کاربر":"Создать","افزودن گروه":"Создать группу","مصرف داده":"Расход трафика","MM/dd/yyyy":"MMMM d, yyy","گره {{name}} با موفقیت حذف شد":"Узел {{name}} успешно удалён","از حذف گره <b>{{name}}</b> مطمئن هستید؟":"Вы уверены, что хотите удалить узел <b>{{name}}</b>?","حذف گره":"Удалить узел","حذف هاست":"Удалить хост","آیا مطمئن هستید که می‌خواهید هاست <b>{{name}}</b> را حذف کنید؟":"Вы уверены, что хотите удалить хост <b>{{name}}</b>?","هاست {{name}} با موفقیت حذف شد":"Хост {{name}} успешно удален","حذف هاست {{name}} ناموفق بود":"Не удалось удалить хост {{name}}","{{count}} هاست با موفقیت حذف شد.":"{{count}} хостов успешно удалено.","حذف هاست‌های انتخاب‌شده ناموفق بود.":"Не удалось удалить выбранные хосты.","حذف هاست‌های انتخاب‌شده":"Удалить выбранные хосты","آیا مطمئن هستید که می‌خواهید {{count}} هاست انتخاب‌شده را حذف کنید؟ این عمل قابل بازگشت نیست.":"Вы уверены, что хотите удалить выбранные хосты ({{count}})? Это действие нельзя отменить.","گره {{name}} با موفقیت بروزرسانی شد":"Узел {{name}} успешно обновлен","ویرایش گره":"Редактировать узел","{{username}} با موفقیت حذف شد.":"{{username}} успешно удалён.","از حذف کاربر <b>{{username}}</b> مطمئن هستید?":"Вы уверены, что хотите удалить пользователя <b>{{username}}</b>?","حذف کاربر":"Удалить пользователя","غیر فعال":"disabled","انقضا":"Истекает","تاریخ انقضا":"Дата истечения","آخرین آنلاین":"Последний онлайн","{{time}} منقضی شده":"Истёк {{time}}","پایان در {{time}}":"Истекает через {{time}}","گزینه‌های مرتب‌سازی":"Параметры сортировки","کمک مالی":"Пожертвование","تنظیمات هاست":"Настройки хоста","خروج":"Выйти","تنظیمات گره‌ها":"Настройки узлов","نمودار گره‌ها":"Использование узлов","ورودی":"Входящий","ورودی‌ها":"Входящие","آدرس":"Адрес","مصرف":"Использование","حالت نمایش":"Режим просмотра","نمای شبکه‌ای":"Сетка","نمای لیستی":"Список","اسم هاست":"Имя хоста","ایجاد":"Создать","تعداد در صفحه":"Элементов на страницу","ورود":"Вход","این فیلد باید پر شود!":"Это поле обязательно для заполнения","وارد حساب خود شوید":"Войдите в свой аккаунт","خوش آمدید, لطفا اطلاعات خود را وارد کنید":"Пожалуйста, введите свои данные","بازگشت به ورود":"Вернуться ко входу","مصرف حافظه":"Память","بعدی":"Вперед","نظارت بر سرورها و کاربران شما":"Следите за своими серверами и пользователями","کنترل جزئیات سرور شما":"Управляйте данными вашего сервера","استفاده از این گره به عنوان هاست تمام ورودی‌ها":"Добавить этот узел, как новый хост для каждого inbound","افزودن گره پاسارگارد":"Добавить новый узел PasarGuard","گره {{name}} با موفقیت افزوده شد":"Узел {{name}} успешно добавлен","گواهی":"Сертификат","پورت API":"API порт","نام":"Название","پورت":"Порт","اتصال مجدد":"Переподключиться","درحال اتصال مجدد...":"Переподключение...","گره‌ها":"Используя PasarGuard-Node, Вы можете масштабировать инфраструктуру, добавляя узлы на разных серверах.","ضریب گره":"Коэффициент использования","در انتظار اتصال":"В ожидании","تکرار":"Дублировать","ایجاد و مدیریت قالب‌ها":"Создавать и управлять шаблонами","گذرواژه":"Пароль","قبلی":"Назад","بازگشت":"Назад","کد QR":"QR-код","لینک اشتراک {{username}}":"Ссылка на подписку {{username}}","اشتراک {{username}}":"Подписка {{username}}","لینک اشتراک":"Ссылка на подписку","کانفیگ‌ها":"Конфигурации","کپی همه":"Копировать все","کپی کانفیگ":"Копировать конфиг","دریافت کانفیگ‌ها ناموفق بود":"Не удалось получить конфигурации","کانفیگی یافت نشد":"Конфигурации не найдены","کانفیگ ناشناخته":"Неизвестная конфигурация","فرمت کد QR":"Формат QR-кода","کانفیگ":"Конфиг","کپی Base64":"Копировать Base64","دانلود وایرگارد":"Скачать WireGuard","بازنشانی":"Отозвать","بازنشانی کل مصرف":"Сбросить расход трафика","بازنشانی مصرف انجام نشد، دوباره تلاش کنید.":"Сброс расхода не удался, пожалуйста, попробуйте ещё раз.","این عمل مصرف تمام کاربران را بازنشانی می‌کند. از این بازنشانی مطمئن هستید؟ این عمل قابل بازگشت نیست!":"Это действие полностью очищает весь расход трафика пользователей. Вы уверены? ЭТО ДЕЙСТВИЕ НЕОБРАТИМО!","مصرف کل با موفقیت بازنشانی شد.":"Расход трафика успешно сброшен.","بازنشانی مصرف تمام کاربران":"Сбросить расход трафика для всех пользователей","از بازنشانی مصرف <b>{{username}}</b> مطئمن هستید؟":"Вы уверены, что хотите сбросить расход трафика для пользователя <b>{{username}}</b>?","مصرف {{username}} با موفقیت بازنشانی شد.":"Расход трафика пользователя {{username}} успешно сброшен.","بازنشانی مصرف کاربر":"Сбросить использование пользователя","بازنشانی اشتراک انجام نشد، لطفاً دوباره امتحان کنید.":"Отзыв подписки не удался, пожалуйста, попробуйте ещё раз.","آیا مطمئن هستید که می خواهید اشتراک «{{username}}» را بازنشانی کنید؟":"Вы уверены, что хотите отозвать подписку для пользователя «{{username}}»?","اشتراک {{username}} با موفقیت بازنشانی شد.":"Подписка пользователя {{username}} успешно отозвана.","بازنشانی اشتراک کاربر":"Отозвать подписку пользователя","جستجو":"Поиск","غیرفعال":"инвалидный","منقضی شده":"Истек","اتمام حد مصرف":"Ограничен","خطا در اتصال":"Ошибка","درحال اتصال":"соединяющий","نام کاربری":"Имя пользователя","کاربران":"Пользователи","کپی شد":"Скопировано","تنظیمات کپی":"Скопировать конфигурации","کپی لینک اشتراک":"Скопировать ссылку на подписку","کد QR اشتراک":"QR-код подписки","کد QR تنظیمات":"QR-код конфигураций","کاربری افزوده نشده است":"В системе нет созданных пользователей","به‌نظر میرسه کاربری که جستجو کردید، وجود ندارد":"Похоже, нет пользователя, соответствующего вашему запросу","هیچ کاربری پیکربندی نشده":"Пользователи не настроены","با ایجاد اولین حساب کاربری شروع کنید.":"Начните с создания первой учетной записи пользователя.","هیچ کاربری با معیارهای جستجوی شما مطابقت ندارد. لطفاً عبارات جستجوی خود را تغییر دهید.":"Нет пользователей, соответствующих вашим критериям поиска. Попробуйте изменить условия поиска.","مجموع":"Всего","سیستم":"Система","کل ترافیک":"Общий трафик","مصرف رم":"Использование RAM","مصرف پردازنده":"Использование CPU","مصرف دیسک":"Использование диска","زمان روشن بودن":"Время работы","هسته":"ядер","داده‌های لحظه‌ای":"Данные в реальном времени","داده‌های تاریخی":"Исторические данные","مشاهده تاریخی":"Просмотр истории","مشاهده لحظه‌ای":"Просмотр в реальном времени","تاریخی":"История","لحظه‌ای":"Реальное время","آمار و نمودارهای زنده برای سرورها و کاربران شما.":"Живая статистика и графики для ваших серверов и пользователей.","داده‌های عملکرد تاریخی سرور برای بازه زمانی انتخاب شده":"Исторические данные о производительности сервера за выбранный период времени","اشتراک‌ها":"Подписки","توزیع کلاینت‌های اشتراک":"Распределение клиентов подписки","جزئیات کلاینت‌های اشتراک":"Детали клиентов подписки","سهم به‌روزرسانی‌های لینک اشتراک بین عامل‌ها و برنامه‌ها.":"Доля обновлений подписки по агентам и приложениям.","به‌طور پیش‌فرض فقط {{limit}} به‌روزرسانی اخیر اشتراک برای هر کاربر نگه داشته می‌شود. برای تغییر، {{env}} را در محیط تنظیم کنید.":"По умолчанию для каждого пользователя хранятся только последние {{limit}} обновлений подписки. Измените это через {{env}} в окружении.","مجموع به‌روزرسانی‌ها":"Всего обновлений","درصد":"Процент","فیلتر بر اساس ادمین":"Фильтр по администратору","یک ادمین را انتخاب کنید":"Выберите администратора","همه ادمین‌ها":"Все администраторы","{{client}} با سهم {{percentage}}٪ پیشتاز به‌روزرسانی‌ها است":"{{client}} лидирует с долей {{percentage}}% обновлений подписки","به‌روزرسانی در دوره زمانی":"Обновления за период","بازه":"Период","این نمودار {{count}} شمع دارد. نمودار ناحیه‌ای از تنظیمات تم معمولاً برای بازه‌های بزرگ روان‌تر است.":"На этом графике {{count}} свечей. Диаграмма-область в настройках темы обычно работает быстрее на больших диапазонах.","استفاده از نمودار ناحیه‌ای":"Использовать область","خودکار":"Авто","انتخاب بازه زمانی":"Выберите временной диапазон","دوره مورد نظر را برای تجزیه و تحلیل داده‌های تاریخی انتخاب کنید":"Выберите диапазон дат выше, чтобы просмотреть статистику использования трафика.","لطفاً یک بازه زمانی برای مشاهده داده‌های تاریخی انتخاب کنید":"Пожалуйста, выберите временной диапазон для просмотра исторических данных","فعالسازی":"Включить","ویرایش هاست":"Редактировать хост","موفق":"Успешно","خطا":"Ошибка","مشاهده آموزش":"Просмотр руководства","مدیریت هسته‌های شما":"Управление вашими ядрами","افزودن هسته":"Добавить ядро","هیچ هسته‌ای موجود نیست. یکی اضافه کنید تا شروع کنید.":"Нет доступных ядер. Добавьте одно, чтобы начать.","پیکربندی هسته «{{name}}» با موفقیت کپی شد":"Конфигурация ядра «{{name}}» успешно скопирована","خطا در کپی پیکربندی هسته «{{name}}»":"Не удалось скопировать конфигурацию ядра «{{name}}»","هسته با موفقیت حذف شد":"Ядро успешно удалено","حذف هسته با خطا مواجه شد":"Не удалось удалить ядро","حذف هسته":"Удалить ядро","هسته یافت نشد":"Ядро не найдено","تاریخ ایجاد":"Дата создания","تغییر وضعیت":"Переключить статус","هسته «{{name}}» با موفقیت تغییر وضعیت داده شد":"Ядро «{{name}}» успешно переключено","تغییر وضعیت هسته «{{name}}» با خطا مواجه شد":"Не удалось переключить ядро «{{name}}»","هسته «{{name}}» با موفقیت حذف شد":"Ядро «{{name}}» успешно удалено","حذف هسته «{{name}}» با خطا مواجه شد":"Не удалось удалить ядро «{{name}}»","آیا مطمئن هستید که می‌خواهید هسته «{{name}}» را حذف کنید؟":"Вы уверены, что хотите удалить «{{name}}»?","{{count}} هسته با موفقیت حذف شد.":"{{count}} ядер успешно удалено.","حذف هسته‌های انتخاب‌شده ناموفق بود.":"Не удалось удалить выбранные ядра.","حذف هسته‌های انتخاب‌شده":"Удалить выбранные ядра","آیا مطمئن هستید که می‌خواهید {{count}} هسته انتخاب‌شده را حذف کنید؟ این عمل قابل بازگشت نیست.":"Вы уверены, что хотите удалить выбранные ядра ({{count}})? Это действие нельзя отменить.","بستن":"Закрыть","مشاهده":"Просмотр","کپی":"Копировать","کپی شد!":"Скопировано!","کلیک برای کپی":"Нажмите, чтобы скопировать","کپی ناموفق بود":"Не удалось скопировать","دانلود کانفیگ ناموفق بود":"Не удалось скачать конфиг","بیشتر":"еще","باز کردن منو":"Развернуть меню","بستن منو":"Свернуть меню","آیا مطمئن هستید که می‌خواهید کاربر «{{name}}» را حذف کنید؟ این عملیات غیرقابل بازگشت است.":"Вы уверены, что хотите удалить пользователя «{{name}}»? Это действие необратимо.","کاربر «{{name}}» با موفقیت حذف شد.":"Пользователь «{{name}}» был успешно удалён.","حذف کاربر «{{name}}» با خطا مواجه شد. {{error}}":"Не удалось удалить пользователя «{{name}}». {{error}}","میزان مصرف ترافیک":"Использование трафика","کل مصرف ترافیک در تمام سرورها":"Общее использование трафика на всех серверах","مصرف در دوره زمانی":"Использование за период","کاربران آنلاین":"Онлайн пользователи","تعداد کاربران":"Количество пользователей","تعداد کاربران آنلاین، منقضی‌شده و محدودشده در طول زمان":"Количество онлайн, истекших и ограниченных пользователей во времени","تغییر وضعیت می‌تواند تاریخچه را نادقیق کند. بازنشانی مصرف فقط با فعال بودن پاک‌سازی داده‌های نمودار تاریخچه را حذف می‌کند.":"Смена статуса может исказить историю. Сброс использования очищает историю графика только при включенной очистке данных.","گروه‌بندی بر اساس گره":"Группировать по узлу","تعداد در بازه":"Количество за период","سلامت پراسس ها":"Состояние воркеров","وضعیت زمان‌بند و پراسس گره":"Статус планировщика и воркера узла","زنده":"В реальном времени","نمایش جزئیات":"Показать детали","بارگذاری سلامت پراسس ها ممکن نیست.":"Не удалось загрузить состояние воркеров.","زمان‌بند":"Планировщик","پراسس گره":"Воркер узла","NATS غیرفعال است. بررسی سلامت پراسس ها در دسترس نیست.":"NATS отключён. Проверки состояния воркеров недоступны.","سالم":"Здоров","در حال اجرا":"Работает","در دسترس":"Доступен","کاهش‌یافته":"Ухудшено","غیرقابل‌دسترسی":"Недоступен","از کار افتاده":"Отключён","نامشخص":"Неизвестно","کاربران محدود شده":"Ограниченные пользователи","کاربران فعال":"Активные пользователи","کاربران غیرفعال":"Отключенные пользователи","کاربران منقضی شده":"Истекшие пользователи","کاربران در انتظار":"Пользователи на удержании","همه کاربران":"Все пользователи","ترافیک سرور":"Трафик сервера","توزیع ترافیک گره ها":"Распределение трафика узлов","هیچ گره ای در دسترس نیست":"Нет доступных узлов","هیچ داده‌ای در دسترس نیست":"Нет данных","یک بازه زمانی را انتخاب کنید تا آمار و روند مصرف ترافیک را مشاهده کنید.":"Выберите временной диапазон для просмотра статистики и трендов использования трафика.","هیچ گره ای در حال حاضر متصل نیست تا توزیع ترافیک را نمایش دهد.":"Нет подключенных узлов для отображения распределения трафика.","در حال دریافت آخرین داده‌های آمار...":"Получение последних статистических данных...","در انتظار داده‌ها":"Ожидание данных","آمار لحظه‌ای در اینجا نمایش داده می‌شود پس از شروع جریان داده‌ها.":"Статистика в реальном времени появится здесь после начала передачи данных.","داده‌ای در بازه انتخابی وجود ندارد":"Нет данных в выбранном диапазоне","هیچ داده ترافیکی برای دوره زمانی انتخابی یافت نشد. بازه زمانی متفاوتی را انتخاب کنید.":"Данные о трафике для выбранного периода времени не найдены. Попробуйте выбрать другой диапазон дат.","یک گره را انتخاب کنید تا آمار دقیق را مشاهده کنید":"Выберите узел для просмотра подробной статистики","نمای کلی از وضعیت و فعالیت حساب‌های کاربری":"Обзор состояния и активности учетных записей пользователей","اتصال با خطا مواجه شد. لطفاً شبکه خود را بررسی کنید و دوباره تلاش کنید.":"Ошибка подключения. Проверьте сеть и попробуйте снова.","بارگذاری آمار با خطا مواجه شد":"Не удалось загрузить статистику","آیا مطمئن هستید که می‌خواهید مصرف «{{name}}» را ریست کنید؟":"Вы уверены, что хотите сбросить использование для «{{name}}»?","بازنشانی مصرف":"Сбросить использование","مصرف کاربر «{{name}}» با موفقیت ریست شد":"Использование пользователя «{{name}}» успешно сброшено","بازنشانی مصرف کاربر «{{name}}» ناموفق بود":"Не удалось сбросить использование пользователя «{{name}}»","هنوز متصل نشده":"Еще не подключен","نسخه هسته":"Версия ядра","نسخه گره":"Версия узла","عملیات موفق بود!":"Операция выполнена успешно!","عملیات ناموفق بود!":"Операция не удалась!","در حال اعمال...":"Применение...","تنظیمات با موفقیت اعمال شد!":"Настройки применены успешно!","تگ‌های ورودی":"Входящие теги","جستجوی ورودی‌ها...":"Поиск входящих...","هیچ ورودی‌ای یافت نشد":"Входящие не найдены","تاریخ":"Дата","مصرف کل":"Общий трафик","ارسال":"Входящий","دریافت":"Исходящий","آمار گره‌ها":"Статистика узлов","آمار دقیق ترافیک هر گره در این بازه زمانی. روی میله‌های نمودار کلیک کنید تا جزئیات بیشتری ببینید.":"Подробная статистика трафика для каждого узла в этот период времени. Нажмите на столбцы диаграммы, чтобы просмотреть дополнительные детали.","برای جزئیات کلیک کنید":"Нажмите для подробностей","برای جزئیات بیشتر کلیک کنید":"Нажмите для дополнительных деталей","انتخاب تاریخ":"Выбрать дату","پاک کردن همه فیلترها":"Очистить все фильтры","همه":"Все","قوانین وضعیت کاربر":"Правила статуса пользователя","هاست‌ها را بر اساس وضعیت‌های کاربرِ پیکربندی‌شده فیلتر می‌کند (فعال، در انتظار، محدود و ...).":"Фильтрует хосты по заданным статусам пользователей (активен, на паузе, ограничен и т.д.).","پاک کردن همه اینباندها":"Очистить все входящие","وضعیت هاست":"Состояние хоста","مشخص می‌کند خود هاست فعال است یا غیرفعال.":"Фильтрует, включен или отключен сам хост.","هاست‌های فعال":"Включенные хосты","هاست‌های غیرفعال":"Отключенные хосты","نمای کلی سیستم":"Обзор системы","بکاپ تلگرام":"Резервная копия Telegram","تنظیمات و ارسال":"Настройки и отправка","تنظیمات بکاپ":"Настройки бэкапа","Scheduler و Backup":"Планировщик и бэкап","تست تلگرام":"Тест Telegram","بررسی اتصال":"Проверка подключения","حساب کاربری":"Аккаунт","مدیریت ورود":"Управление входом","پایان نشست":"Завершить сеанс","منوی اصلی":"Главное меню","مدیریت Backupها":"Управление бэкапами","Backupهای موجود را مشاهده، دانلود یا حذف کنید.":"Просмотр, скачивание или удаление бэкапов.","هنوز Backupای پیدا نشد.":"Бэкапы не найдены.","اطلاعات Backup":"Информация о бэкапе","تعداد Backup":"Количество бэкапов","حجم کل Backupها":"Общий размер бэкапов","آخرین Backup":"Последний бэкап","هنوز Backup ساخته نشده":"Бэкапы ещё не созданы","قابل دریافت نیست":"Недоступно","لینک پنل":"Ссылка панели","وضعیت پنل":"Статус панели","مصرف واقعی Node":"Реальный трафик Node","تنظیم نشده":"Не настроено","متوقف":"Остановлено","Bot Token":"Bot Token","Chat ID":"Chat ID","Topic ID":"Topic ID","Proxy":"Прокси","بدون Proxy":"Без прокси","تنظیمات Telegram":"Настройки Telegram","ارسال تست":"Тестовая отправка","تنظیمات Backup":"Настройки бэкапа","مدیریت Backup":"Управление бэкапом","شروع Backup دستی":"Запустить бэкап вручную","ارسال تست پیام به Telegram":"Отправить тестовое сообщение в Telegram","قبل از فعال‌کردن Scheduler اتصال را بررسی کنید.":"Проверьте подключение перед включением планировщика.","Backup بعدی":"Следующий бэкап","Backup هفت روز اخیر":"Бэкапы за 7 дней","فضای دیسک":"Дисковое пространство","وضعیت سرویس‌ها":"Состояние сервисов","منابع سرور مجازی":"Ресурсы виртуального сервера","CPU · RAM · Disk · پایش لحظه‌ای":"CPU · RAM · Disk · Мониторинг в реальном времени","وضعیت لحظه‌ای سرور":"Состояние сервера в реальном времени","فعالیت‌های اخیر":"Последние действия","آخرین فعالیت‌ها":"Последние действия","تعداد فعالیت‌های اخیر":"Количество последних действий","همین الان":"Только что","دقیقه پیش":"мин. назад","ساعت پیش":"ч. назад","روز پیش":"дн. назад","روز":"дн.","ساعت":"ч.","دقیقه":"мин.","به‌روزرسانی":"Обновлено","اکنون":"Сейчас","مدیریت خصوصی":"Приватное администрирование","این بخش فقط برای مدیر اصلی است.":"Этот раздел доступен только главному администратору.","ورود به پنل":"Вход в панель","ورود امن":"Безопасный вход","راه‌اندازی اولیه":"Первичная настройка","ساخت رمز و ورود":"Создать пароль и войти","رمز مدیر":"Пароль администратора","تکرار رمز":"Повтор пароля","رمز جدید":"Новый пароль","تکرار رمز جدید":"Повтор нового пароля","تغییر رمز":"Изменить пароль","مرکز مدیریت":"Центр управления","شخصی‌سازی کامل ظاهر، داشبورد و اجزای وب‌پنل.":"Полная настройка внешнего вида, панели и компонентов.","شخصی‌سازی":"Настройка","شش تم آماده":"Шесть готовых тем","کاربر عادی هیچ گزینه‌ای برای انتخاب تم نمی‌بیند.":"Обычные пользователи не видят выбор темы.","ظاهر":"Внешний вид","رنگ اصلی":"Основной цвет","رنگ دوم":"Вторичный цвет","Background":"Фон","Logo URL":"URL логотипа","عرض Logo":"Ширина логотипа","شدت Glow":"Интенсивность свечения","Glow و حرکت":"Свечение и анимация","سرعت انیمیشن":"Скорость анимации","متن و برند":"Текст и бренд","نام پنل":"Название панели","عنوان داشبورد":"Заголовок панели","عنوان بکاپ":"Заголовок бэкапа","عنوان حجم مصرفی":"Заголовок трафика","عنوان فعالیت‌های اخیر":"Заголовок последних действий","چیدمان و تجربه کاربر":"Макет и UX","عرض محتوای داشبورد":"Ширина содержимого","فاصله بین کارت‌ها":"Расстояние между карточками","بخش‌های داشبورد":"Разделы панели","حجم مصرفی":"Использование трафика","آمار سرور":"Статистика сервера","دکمه سفارشی":"Пользовательская кнопка","دکمه‌های سفارشی":"Пользовательские кнопки","آیکون":"Иконка","لینک":"Ссылка","افزودن":"Добавить","ذخیره تغییرات":"Сохранить изменения","کاستوم":"Пользовательская","لایت":"Светлая","دارک":"Тёмная","بکاپ دستی":"Ручной бэкап","زمان‌بندی خودکار":"Автопланировщик","بازه Backup (ساعت)":"Интервал бэкапа (часы)","شامل PG-Node شود":"Включить PG-Node","ذخیره و شروع":"Сохранить и запустить","توقف":"Остановить","همین حالا یک Backup کامل بگیرید.":"Создать полный бэкап сейчас.","یک پیام آزمایشی با تنظیمات فعلی ارسال می‌شود.":"Будет отправлено тестовое сообщение с текущими настройками.","ارسال پیام تست":"Отправить тестовое сообщение","پیام تست با موفقیت ارسال شد.":"Тестовое сообщение отправлено.","ارسال پیام تست ناموفق بود.":"Не удалось отправить тестовое сообщение.","Scheduler ذخیره و شروع شد.":"Планировщик сохранён и запущен.","Scheduler ذخیره شد ولی شروع آن با خطا مواجه شد.":"Планировщик сохранён, но запуск завершился ошибкой.","تنظیمات عمومی ذخیره شد.":"Общие настройки сохранены.","تنظیمات Telegram با موفقیت ذخیره شد.":"Настройки Telegram сохранены.","تنظیمات حساب با موفقیت ذخیره شد.":"Настройки аккаунта сохранены.","تنظیمات با موفقیت ذخیره شد و روی کل وب‌پنل اعمال شد.":"Настройки сохранены и применены ко всей панели.","آفلاین":"Офлайн","نامعتبر":"Недействительно"}};
const attrNames=["title","aria-label","placeholder","data-label","data-title"];
function norm(s){return String(s||"").replace(/\u200c/g,"‌").replace(/\s+/g," ").trim();}
function translateText(raw,dict){
 const s=norm(raw); if(!s)return raw;
 if(Object.prototype.hasOwnProperty.call(dict,s))return dict[s];
 let out=s;
 Object.keys(dict).sort((a,b)=>b.length-a.length).forEach(k=>{if(k.length>1&&out.includes(k))out=out.split(k).join(dict[k]);});
 return out===s?raw:out;
}
...
function translateNode(root,lang){
 ...
 nodes.forEach(node=>{
  if(node.nodeType===Node.TEXT_NODE){
   const p=node.parentElement;if(!p||p.closest("script,style,noscript"))return;
   const current=node.nodeValue;if(!current||!current.trim())return;
   if(!textOriginals.has(node))textOriginals.set(node,current);
   const original=textOriginals.get(node);
   node.nodeValue=translateText(original,dict);
  }else{
   if(node.matches("input,textarea,select,option,script,style,noscript"))return;
   attrNames.forEach(a=>{if(node.hasAttribute(a)){const key="data-i18n-"+a;if(!node.hasAttribute(key))node.setAttribute(key,node.getAttribute(a));node.setAttribute(a,translateText(node.getAttribute(key),dict));}});
  }
 });
}
function setLanguage(lang){
 ...
 if(toggle){toggle.setAttribute("aria-label",lang==="fa"?"تغییر زبان":lang==="ru"?"Сменить язык":"Change language");toggle.title=lang==="fa"?"زبان":lang==="ru"?"Язык":"Language";}
...
setLanguage(current);
const observer=new MutationObserver(()=>{if(document.body.dataset.i18nBusy)return;document.body.dataset.i18nBusy="1";setTimeout(()=>{delete document.body.dataset.i18nBusy;try{translateNode(document.body,localStorage.getItem(LANG_KEY)||"en")}catch(e){}},0);});
