...
.language-picker{position:relative}.language-toggle{min-width:46px;width:46px;height:42px;padding:0;border:1px solid var(--line);border-radius:14px;color:var(--text);background:var(--glass2);backdrop-filter:blur(18px);cursor:pointer;font-size:18px;transition:.25s ease}.language-toggle:hover{transform:translateY(-2px);border-color:rgba(34,211,238,.45);box-shadow:0 10px 30px rgba(34,211,238,.14)}.language-svg{width:22px;height:22px;display:block;fill:none;stroke:currentColor;stroke-width:1.7;stroke-linecap:round;stroke-linejoin:round;filter:drop-shadow(0 0 4px currentColor) drop-shadow(0 0 9px currentColor);transition:.25s ease}.language-toggle:hover .language-svg{transform:rotate(10deg) scale(1.06);filter:drop-shadow(0 0 5px currentColor) drop-shadow(0 0 12px currentColor)}.lang-mark{display:inline-grid;place-items:center;min-width:28px;height:22px;padding:0 5px;border-radius:7px;font-size:9px;font-weight:900;letter-spacing:.4px;background:linear-gradient(135deg,rgba(34,211,238,.14),rgba(139,92,246,.18));border:1px solid rgba(34,211,238,.16);box-shadow:0 0 12px rgba(34,211,238,.08)}.language-menu button{justify-content:flex-start}.light .language-svg{filter:drop-shadow(0 0 3px rgba(109,40,217,.32))}.light .lang-mark{background:linear-gradient(135deg,rgba(124,58,237,.08),rgba(219,39,119,.10));border-color:rgba(109,40,217,.15);box-shadow:0 0 12px rgba(109,40,217,.08)}.language-menu{position:absolute;top:50px;right:0;z-index:131;min-width:155px;padding:7px;border:1px solid var(--line);border-radius:16px;background:rgba(12,14,24,.94);backdrop-filter:blur(22px) saturate(150%);-webkit-backdrop-filter:blur(22px) saturate(150%);box-shadow:0 18px 50px rgba(0,0,0,.35)}.language-menu[hidden]{display:none}.language-menu button{width:100%;display:flex;align-items:center;gap:9px;padding:10px 11px;border:0;border-radius:11px;background:transparent;color:var(--text);cursor:pointer;font:inherit;text-align:right}.language-menu button:hover,.language-menu button.active{background:rgba(124,58,237,.12)}body.light .language-menu{background:rgba(255,255,255,.96);border-color:rgba(124,58,237,.18);box-shadow:0 18px 50px rgba(124,58,237,.18)}body.light .language-menu button{color:#26162d}body.light .language-menu button:hover,body.light .language-menu button.active{background:rgba(124,58,237,.08)}
...
def ui_language_menu():
    globe = '<svg class="language-svg" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><circle cx="12" cy="12" r="8.7"/><path d="M3.6 9.2h16.8M3.6 14.8h16.8M12 3.3c2.2 2.3 3.2 5.2 3.2 8.7s-1 6.4-3.2 8.7c-2.2-2.3-3.2-5.2-3.2-8.7Z"/></svg>'
    return f'<div class="language-picker"><button class="language-toggle" id="languageToggle" type="button" aria-label="تغییر زبان" title="زبان">{globe}</button><div class="language-menu" id="languageMenu" hidden><button type="button" data-lang-choice="fa"><span class="lang-mark">FA</span><span>فارسی</span></button><button type="button" data-lang-choice="en"><span class="lang-mark">EN</span><span>English</span></button><button type="button" data-lang-choice="ru"><span class="lang-mark">RU</span><span>Русский</span></button></div></div>'
...
def ui_language_script():
    ...
    return r"""const LANG_KEY="idontpg-lang";
const maps={
 en:{"داشبورد":"Dashboard","نمای کلی سیستم":"System overview","بکاپ تلگرام":"Telegram Backup","تنظیمات و ارسال":"Settings & delivery","تنظیمات بکاپ":"Backup Settings","Scheduler و Backup":"Scheduler & Backup","تست تلگرام":"Telegram Test","بررسی اتصال":"Connection check","حساب کاربری":"Account","مدیریت ورود":"Login management","خروج":"Logout","پایان نشست":"End session","منوی اصلی":"Main menu","کنترل کامل Backup":"Complete Backup Control","همه‌چیز برای مدیریت Backup، ارسال به Telegram و زمان‌بندی خودکار، داخل یک پنل شیشه‌ای و سریع.":"Everything for backup management, Telegram delivery and automatic scheduling in one fast glass panel.","Backup Control Center · durwinam":"Backup Control Center · durwinam","اطلاعات Backup":"Backup Information","Panel Information":"Panel Information","Telegram Backup":"Telegram Backup","Backup Controls":"Backup Controls","Manual Backup":"Manual Backup","Next Scheduled Backup":"Next Scheduled Backup","Disk Monitor":"Disk Monitor","Service status":"Service status","System status":"System status","Web Panel":"Web Panel","PasarGuard":"PasarGuard","Telegram":"Telegram","Scheduler":"Scheduler","Disk":"Disk","Unknown":"Unknown","Configured":"Configured","No proxy":"No proxy","Custom link":"Custom link","اطلاعات بکاپ":"Backup Information","تعداد Backup":"Backup count","حجم کل Backupها":"Total backup size","آخرین Backup":"Latest backup","هنوز Backup ساخته نشده":"No backup created yet","هنوز Backupای پیدا نشد.":"No backups found.","مدیریت Backupها":"Manage Backups","لینک پنل":"Panel link","وضعیت پنل":"Panel status","مصرف واقعی Node":"Real Node usage","تنظیم نشده":"Not configured","در حال اجرا":"Running","فعال":"Active","متوقف":"Stopped","آنلاین":"Online","آفلاین":"Offline","تنظیم شده":"Configured","قابل دریافت نیست":"Unavailable","Bot Token":"Bot Token","Chat ID":"Chat ID","Topic ID":"Topic ID","Proxy":"Proxy","بدون Proxy":"No Proxy","تنظیمات Telegram":"Telegram Settings","ارسال تست":"Send Test","مدیریت Backup":"Manage Backup","شروع Backup دستی":"Run Manual Backup","ارسال تست پیام به Telegram":"Send a Test Message to Telegram","قبل از فعال‌کردن Scheduler اتصال را بررسی کنید.":"Check the connection before enabling the scheduler.","با Chat ID و Topic فعلی ارسال می‌شود.":"Sent using the current Chat ID and Topic.","Backup بعدی":"Next Backup","Backup هفت روز اخیر":"Backup — Last 7 Days","Backup Activity":"Backup Activity","فضای دیسک":"Disk Space","وضعیت سرویس‌ها":"Service status","Health Check":"Health Check","منابع سرور مجازی":"Virtual Server Resources","CPU · RAM · Disk · پایش لحظه‌ای":"CPU · RAM · Disk · Live monitoring","وضعیت لحظه‌ای سرور":"Live server status","LIVE TELEMETRY":"LIVE TELEMETRY","به‌روزرسانی":"Updated","اکنون":"Now","استفاده‌شده":"Used","آزاد":"Free","فعالیت‌های اخیر":"Recent Activity","آخرین فعالیت‌ها":"Recent Activity","Recent Activity":"Recent Activity","تعداد فعالیت‌های اخیر":"Recent activity count","هنوز فعالیتی ثبت نشده.":"No activity recorded yet","همین الان":"Just now","دقیقه پیش":"minutes ago","ساعت پیش":"hours ago","روز پیش":"days ago","روز":"days","ساعت":"hours","دقیقه":"minutes","مدیریت خصوصی":"Private Administration","این بخش فقط برای مدیر اصلی است.":"This area is for the main administrator only.","ورود به پنل":"Panel Login","برای ورود، نام کاربری و رمز عبور ادمین را وارد کنید.":"Enter the admin username and password to continue.","ورود امن":"Secure Login","راه‌اندازی اولیه":"Initial Setup","ساخت رمز و ورود":"Create Password & Login","ساخت حساب و ورود":"Create Account & Login","نام کاربری ادمین":"Admin username","رمز ادمین":"Admin password","نام کاربری":"Username","رمز عبور":"Password","تکرار رمز":"Confirm password","رمز جدید":"New password","تکرار رمز جدید":"Confirm new password","تغییر رمز":"Change Password","ورود به مدیریت":"Enter Administration","مرکز مدیریت":"Control Center","شخصی‌سازی":"Customization","شخصی‌سازی کامل ظاهر، داشبورد و اجزای وب‌پنل.":"Customize the complete look, dashboard and web-panel components.","شش تم آماده":"Six Ready Themes","کاربر عادی هیچ گزینه‌ای برای انتخاب تم نمی‌بیند.":"Regular users cannot choose the theme.","ظاهر":"Appearance","رنگ اصلی":"Primary color","رنگ دوم":"Secondary color","Background":"Background","Logo URL":"Logo URL","عرض Logo":"Logo width","شدت Glow":"Glow intensity","Glow و حرکت":"Glow & Motion","سرعت انیمیشن":"Animation speed","اندازه فونت":"Font size","متن و برند":"Text & Branding","نام پنل":"Panel name","عنوان داشبورد":"Dashboard title","توضیح داشبورد":"Dashboard description","عنوان بکاپ":"Backup title","عنوان حجم مصرفی":"Traffic title","عنوان فعالیت":"Activity title","عنوان فعالیت‌های اخیر":"Recent activity title","عنوان مصرف":"Traffic title","چیدمان و تجربه کاربر":"Layout & User Experience","عرض محتوای داشبورد":"Dashboard content width","فاصله بین کارت‌ها":"Card spacing","بخش‌های داشبورد":"Dashboard sections","حجم مصرفی":"Traffic usage","آمار سرور":"Server statistics","دکمه سفارشی":"Custom Button","دکمه‌های سفارشی":"Custom Buttons","نام":"Name","آیکون":"Icon","لینک":"Link","افزودن":"Add","حذف":"Delete","ذخیره تغییرات":"Save Changes","بازگشت":"Back","کاستوم":"Custom","لایت":"Light","دارک":"Dark","نمایش بخش‌ها":"Visible sections","بکاپ":"Backup","بکاپ دستی":"Manual Backup","زمان‌بندی خودکار":"Automatic Scheduler","بازه Backup (ساعت)":"Backup interval (hours)","شامل PG-Node شود":"Include PG-Node","ذخیره و شروع":"Save & Start","توقف":"Stop","همین حالا یک Backup کامل بگیرید.":"Create a full backup now.","یک پیام آزمایشی با تنظیمات فعلی ارسال می‌شود.":"A test message will be sent using the current settings.","ارسال پیام تست":"Send Test Message","تنظیمات عمومی":"General Settings","تنظیمات حساب":"Account Settings","تنظیمات عمومی ذخیره شد.":"General settings saved.","تنظیمات Telegram با موفقیت ذخیره شد.":"Telegram settings saved successfully.","تنظیمات حساب با موفقیت ذخیره شد.":"Account settings saved successfully.","تنظیمات با موفقیت ذخیره شد و روی کل وب‌پنل اعمال شد.":"Settings saved and applied across the web panel.","Scheduler ذخیره و شروع شد.":"Scheduler saved and started.","Scheduler ذخیره شد ولی شروع آن با خطا مواجه شد.":"Scheduler saved, but starting it failed.","پیام تست با موفقیت ارسال شد.":"Test message sent successfully.","ارسال پیام تست ناموفق بود.":"Test message failed.","Backup با موفقیت ساخته و ارسال شد.":"Backup created and sent successfully.","Backup ناموفق بود.":"Backup failed.","Backup ساخته شد:":"Backup created:","Backup و ارسال به Telegram موفق بود":"Backup and Telegram delivery succeeded","Backup و ارسال Telegram ناموفق بود":"Backup created, but Telegram delivery failed","انتخاب تم":"Theme selection","تغییر زبان":"Change language","زبان":"Language","باز کردن منو":"Open menu","بستن منو":"Close menu","لینک سفارشی":"Custom link","دکمه":"Button","در دسترس نیست":"Unavailable"},
 ru:{"داشبورد":"Панель","نمای کلی سیستم":"Обзор системы","بکاپ تلگرام":"Резервная копия Telegram","تنظیمات و ارسال":"Настройки и отправка","تنظیمات بکاپ":"Настройки бэкапа","Scheduler و Backup":"Планировщик и бэкап","تست تلگرام":"Тест Telegram","بررسی اتصال":"Проверка подключения","حساب کاربری":"Аккаунт","مدیریت ورود":"Управление входом","خروج":"Выйти","پایان نشست":"Завершить сеанс","منوی اصلی":"Главное меню","کنترل کامل Backup":"Полный контроль бэкапов","همه‌چیز برای مدیریت Backup، ارسال به Telegram و زمان‌بندی خودکار، داخل یک پنل شیشه‌ای و سریع.":"Управление бэкапами, отправка в Telegram и автопланировщик — всё в одной быстрой стеклянной панели.","Backup Control Center · durwinam":"Центр управления бэкапами · durwinam","اطلاعات Backup":"Информация о бэкапе","Panel Information":"Информация о панели","Telegram Backup":"Резервная копия Telegram","Backup Controls":"Управление бэкапами","Manual Backup":"Ручной бэкап","Next Scheduled Backup":"Следующий запланированный бэкап","Disk Monitor":"Мониторинг диска","Service status":"Состояние сервисов","System status":"Состояние системы","Web Panel":"Веб-панель","Telegram":"Telegram","Scheduler":"Планировщик","Disk":"Диск","Unknown":"Неизвестно","Configured":"Настроено","No proxy":"Без прокси","Custom link":"Пользовательская ссылка","اطلاعات بکاپ":"Информация о бэкапе","تعداد Backup":"Количество бэкапов","حجم کل Backupها":"Общий размер бэкапов","آخرین Backup":"Последний бэкап","هنوز Backup ساخته نشده":"Бэкапы ещё не созданы","هنوز Backupای پیدا نشد.":"Бэкапы не найдены.","مدیریت Backupها":"Управление бэкапами","لینک پنل":"Ссылка панели","وضعیت پنل":"Статус панели","مصرف واقعی Node":"Реальный трафик Node","تنظیم نشده":"Не настроено","در حال اجرا":"Работает","فعال":"Активно","متوقف":"Остановлено","آنلاین":"Онлайн","آفلاین":"Офлайн","تنظیم شده":"Настроено","قابل دریافت نیست":"Недоступно","Bot Token":"Bot Token","Chat ID":"Chat ID","Topic ID":"Topic ID","Proxy":"Прокси","بدون Proxy":"Без прокси","تنظیمات Telegram":"Настройки Telegram","ارسال تست":"Тестовая отправка","مدیریت Backup":"Управление бэкапом","شروع Backup دستی":"Запустить бэкап вручную","ارسال تست پیام به Telegram":"Отправить тестовое сообщение в Telegram","قبل از فعال‌کردن Scheduler اتصال را بررسی کنید.":"Проверьте подключение перед включением планировщика.","با Chat ID و Topic فعلی ارسال می‌شود.":"Отправляется с текущими Chat ID и Topic.","Backup بعدی":"Следующий бэкап","Backup هفت روز اخیر":"Бэкапы за 7 дней","فضای دیسک":"Дисковое пространство","وضعیت سرویس‌ها":"Состояние сервисов","Health Check":"Проверка состояния","منابع سرور مجازی":"Ресурсы виртуального сервера","CPU · RAM · Disk · پایش لحظه‌ای":"CPU · RAM · Disk · Мониторинг в реальном времени","وضعیت لحظه‌ای سرور":"Состояние сервера в реальном времени","LIVE TELEMETRY":"ТЕЛЕМЕТРИЯ В РЕАЛЬНОМ ВРЕМЕНИ","به‌روزرسانی":"Обновлено","اکنون":"Сейчас","استفاده‌شده":"Использовано","آزاد":"Свободно","فعالیت‌های اخیر":"Последние действия","آخرین فعالیت‌ها":"Последние действия","Recent Activity":"Последние действия","تعداد فعالیت‌های اخیر":"Количество последних действий","هنوز فعالیتی ثبت نشده.":"Действия ещё не зарегистрированы.","همین الان":"Только что","دقیقه پیش":"мин. назад","ساعت پیش":"ч. назад","روز پیش":"дн. назад","روز":"дн.","ساعت":"ч.","دقیقه":"мин.","مدیریت خصوصی":"Приватное администрирование","این بخش فقط برای مدیر اصلی است.":"Этот раздел доступен только главному администратору.","ورود به پنل":"Вход в панель","برای ورود، نام کاربری و رمز عبور ادمین را وارد کنید.":"Введите имя пользователя и пароль администратора.","ورود امن":"Безопасный вход","راه‌اندازی اولیه":"Первичная настройка","ساخت رمز و ورود":"Создать пароль и войти","ساخت حساب و ورود":"Создать аккаунт и войти","نام کاربری ادمین":"Имя пользователя администратора","رمز ادمین":"Пароль администратора","نام کاربری":"Имя пользователя","رمز عبور":"Пароль","تکرار رمز":"Повтор пароля","رمز جدید":"Новый пароль","تکرار رمز جدید":"Повтор нового пароля","تغییر رمز":"Изменить пароль","ورود به مدیریت":"Войти в администрирование","مرکز مدیریت":"Центр управления","شخصی‌سازی":"Настройка","شخصی‌سازی کامل ظاهر، داشبورد و اجزای وب‌پنل.":"Полная настройка внешнего вида, панели и компонентов.","شش تم آماده":"Шесть готовых тем","کاربر عادی هیچ گزینه‌ای برای انتخاب تم نمی‌بیند.":"Обычные пользователи не видят выбор темы.","ظاهر":"Внешний вид","رنگ اصلی":"Основной цвет","رنگ دوم":"Вторичный цвет","Background":"Фон","Logo URL":"URL логотипа","عرض Logo":"Ширина логотипа","شدت Glow":"Интенсивность свечения","Glow و حرکت":"Свечение и анимация","سرعت انیمیشن":"Скорость анимации","اندازه فونت":"Размер шрифта","متن و برند":"Текст и бренд","نام پنل":"Название панели","عنوان داشبورد":"Заголовок панели","توضیح داشبورد":"Описание панели","عنوان بکاپ":"Заголовок бэкапа","عنوان حجم مصرفی":"Заголовок трафика","عنوان فعالیت":"Заголовок действий","عنوان فعالیت‌های اخیر":"Заголовок последних действий","عنوان مصرف":"Заголовок трафика","چیدمان و تجربه کاربر":"Макет и UX","عرض محتوای داشبورد":"Ширина содержимого панели","فاصله بین کارت‌ها":"Расстояние между карточками","بخش‌های داشبورد":"Разделы панели","حجم مصرفی":"Использование трафика","آمار سرور":"Статистика сервера","دکمه سفارشی":"Пользовательская кнопка","دکمه‌های سفارشی":"Пользовательские кнопки","نام":"Название","آیکون":"Иконка","لینک":"Ссылка","افزودن":"Добавить","حذف":"Удалить","ذخیره تغییرات":"Сохранить изменения","بازگشت":"Назад","کاستوم":"Пользовательская","لایت":"Светлая","دارک":"Тёмная","نمایش بخش‌ها":"Видимые разделы","بکاپ":"Бэкап","بکاپ دستی":"Ручной бэкап","زمان‌بندی خودکار":"Автопланировщик","بازه Backup (ساعت)":"Интервал бэкапа (часы)","شامل PG-Node شود":"Включить PG-Node","ذخیره و شروع":"Сохранить и запустить","توقف":"Остановить","همین حالا یک Backup کامل بگیرید.":"Создать полный бэкап сейчас.","یک پیام آزمایشی با تنظیمات فعلی ارسال می‌شود.":"Будет отправлено тестовое сообщение с текущими настройками.","ارسال پیام تست":"Отправить тестовое сообщение","تنظیمات عمومی":"Общие настройки","تنظیمات حساب":"Настройки аккаунта","تنظیمات عمومی ذخیره شد.":"Общие настройки сохранены.","تنظیمات Telegram با موفقیت ذخیره شد.":"Настройки Telegram сохранены.","تنظیمات حساب با موفقیت ذخیره شد.":"Настройки аккаунта сохранены.","تنظیمات با موفقیت ذخیره شد و روی کل وب‌پنل اعمال شد.":"Настройки сохранены и применены ко всей панели.","Scheduler ذخیره و شروع شد.":"Планировщик сохранён и запущен.","Scheduler ذخیره شد ولی شروع آن با خطا مواجه شد.":"Планировщик сохранён, но запуск завершился ошибкой.","پیام تست با موفقیت ارسال شد.":"Тестовое сообщение отправлено.","ارسال پیام تست ناموفق بود.":"Не удалось отправить тестовое сообщение.","Backup با موفقیت ساخته و ارسال شد.":"Бэкап успешно создан и отправлен.","Backup ناموفق بود.":"Ошибка создания бэкапа.","Backup ساخته شد:":"Бэкап создан:","Backup و ارسال به Telegram موفق بود":"Бэкап и отправка в Telegram выполнены успешно","Backup و ارسال Telegram ناموفق بود":"Бэкап создан, но отправка в Telegram не удалась","انتخاب تم":"Выбор темы","تغییر زبان":"Сменить язык","زبان":"Язык","باز کردن منو":"Открыть меню","بستن منو":"Закрыть меню","لینک سفارشی":"Пользовательская ссылка","دکمه":"Кнопка","در دسترس نیست":"Недоступно"}
};
function translateValue(raw,dict){let out=raw;Object.keys(dict).sort((a,b)=>b.length-a.length).forEach(k=>{if(k.length>1&&out.includes(k))out=out.split(k).join(dict[k]);});return out;}
function setLanguage(lang){
 if(!maps[lang])lang="en";
 const root=document.documentElement,body=document.body;root.lang=lang;root.dir=lang==="fa"?"rtl":"ltr";
 ...
 const walker=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT);const list=[];while(walker.nextNode())list.push(walker.currentNode);
 list.forEach(node=>{const p=node.parentElement;if(!p||/^(SCRIPT|STYLE|NOSCRIPT|TEXTAREA)$/.test(p.tagName))return;const raw=p.dataset.i18nOriginalText||node.nodeValue;if(!raw||!raw.trim())return;p.dataset.i18nOriginalText=raw;node.nodeValue=translateValue(raw,dict);});
 document.querySelectorAll("input[placeholder],textarea[placeholder],option,[title],[aria-label]").forEach(el=>{
  ["placeholder","title","aria-label"].forEach(attr=>{if(!el.hasAttribute(attr))return;const key="i18n"+attr;const raw=el.dataset[key]||el.getAttribute(attr);el.dataset[key]=raw;el.setAttribute(attr,translateValue(raw,dict));});
  if(el.tagName==="OPTION"){const raw=el.dataset.i18nOption||el.textContent;el.dataset.i18nOption=raw;el.textContent=translateValue(raw,dict);}
 });
 const title=document.querySelector("title");if(title){const raw=title.dataset.i18nOriginal||title.textContent;title.dataset.i18nOriginal=raw;title.textContent=translateValue(raw,dict)}
 try{localStorage.setItem(LANG_KEY,lang)}catch(e){}
}
const lb=document.getElementById("languageToggle"),lm=document.getElementById("languageMenu");
let current="en";try{current=localStorage.getItem(LANG_KEY)||"en"}catch(e){}if(!maps[current])current="en";
...
def _ui_logo_src():
    try:
        value = str(idont_load_ui_settings().get("logo",{}).get("url","") or "").strip()
