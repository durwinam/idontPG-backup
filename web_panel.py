...
def send_archive(core, archive, c, caption):
    ...
    if not archive or not os.path.exists(archive):
        return False, "فایل پشتیبان ساخته نشد."
    parts_to_remove = []
    try:
        ...
        if hasattr(core, "_split_file_into_chunks"):
            ...
            chunks = []
        if not chunks:
            return False, "تقسیم فایل بزرگ برای Telegram در هسته پشتیبان در دسترس نیست."
        ...
        return True, f"پشتیبان در {len(chunks)} قسمت ارسال شد."
    finally:
...
def make_backup(send=True):
    ...
    try:
        ...
        if not send:
            _record_activity("پشتیبان‌گیری دستی ساخته شد", "ok")
            return True, f"پشتیبان ساخته شد: {archive}"
        ok, msg = send_archive(core, archive, c, f"idontPG-backup · {time.strftime('%Y-%m-%d %H:%M:%S')}")
        _record_activity("پشتیبان‌گیری و ارسال به تلگرام موفق بود" if ok else "پشتیبان ساخته شد ولی ارسال تلگرام ناموفق بود", "ok" if ok else "bad")
        return ok, msg
    except Exception as exc:
        _record_activity("پشتیبان‌گیری ناموفق بود", "bad")
        raise
...
def _resource_chart_html():
  ...
  <div class="resource-summary">
    <div class="resource-stat cpu">{ui_icon("cpu")}<div><small>بار پردازنده</small><strong id="cpuValue">CPUVAL%</strong></div><i></i></div>
    <div class="resource-stat ram">{ui_icon("ram")}<div><small>رم مصرفی</small><strong id="ramValue">RAMVAL%</strong></div><i></i></div>
    <div class="resource-stat disk">{ui_icon("disk")}<div><small>دیسک مصرفی</small><strong id="diskValue">DISKVAL%</strong></div><i></i></div>
  </div>
  <div class="resource-plot">
    <div class="plot-head"><div><span class="plot-kicker">پایش زنده</span><b>وضعیت لحظه‌ای سرور</b></div><span class="live-dot"><i></i> زنده</span></div>
    <div class="plot-body">
      ...
      <div class="chart-legend"><span class="legend cpu"><i></i> پردازنده</span><span class="legend ram"><i></i> رم</span><span class="legend disk"><i></i> دیسک</span><span class="updated" id="resourceUpdated">اکنون</span></div>
    </div>
  </div>
</div>
<script>
...
def get_health_info(c,panel_info=None):
    ...
    return [('وب‌پنل',True,'در حال اجرا'),('PasarGuard',panel_info.get('status')=='Online',panel_info.get('status','Unknown')),('Telegram',telegram_ok,'تنظیم شده' if telegram_ok else 'تنظیم نشده'),('Scheduler',scheduler=='active','فعال' if scheduler=='active' else 'متوقف'),('Disk',float(disk.get('percent',0))<90,f"{disk.get('percent',0)}% استفاده")]
...
body.light .drawer{background:linear-gradient(145deg,rgba(255,255,255,.96),rgba(248,240,255,.92))!important;border-color:rgba(124,58,237,.16)!important}html[dir="ltr"] .updated{margin-left:auto;margin-right:0}html[dir="ltr"] .drawer{left:auto;right:0}html[dir="ltr"] .plot-body{padding-left:38px;padding-right:0}html[dir="ltr"] .y-axis{left:0;right:auto;align-items:flex-end}
@media(max-width:720px){.top-actions{flex-wrap:wrap}.resource-stat{min-height:68px}}
...
<label>پس‌زمینه</label><input name="background" value="%s">
<label>آدرس لوگو</label><input name="logo_url" value="%s">
...
<div class="row"><b>وضعیت</b><b>آیکون</b><b>رنگ</b><b>درخشش</b><b>سرعت</b></div>
%s</div>
...
class Handler(BaseHTTPRequestHandler):
    ...
    def admin_login_page(self, error=""):
        ...
        return page("ورود مدیریت", body, False)
    ...
    def do_GET(self):
        ...
        if not c.get("password_hash"):
            ...
            self.send_html(page("راه‌اندازی اولیه", body, False)); return
        if path == "/login":
...
<div class="glass wide admin-section"><h3 class="title">😀 Emoji Status</h3><div class="emoji-grid">'''
            ...
            self.send_html(page("مدیریت", body)); return
...
<div class="grid">
<article class="glass card"><div class="card-head"><div style="display:flex;gap:12px">{ui_icon("backup", "card-icon")}<div><h3 class="title">{html.escape(ui_now.get("texts",{}).get("backup_title","اطلاعات Backup"))}</h3><p class="sub">اطلاعات پشتیبان‌گیری</p></div></div></div>
<div class="meta"><div class="meta-row"><span>{ui_icon("backup", "meta-icon")} تعداد پشتیبان‌ها</span><strong>{backup_info["count"]}</strong></div><div class="meta-row"><span>{ui_icon("disk", "meta-icon")} حجم کل پشتیبان‌ها</span><strong>{html.escape(backup_info["size"])}</strong></div><div class="meta-row"><span>{ui_icon("clock", "meta-icon")} آخرین پشتیبان</span><strong title="{html.escape(backup_info["latest"])}">{html.escape(backup_info["latest_time"])}</strong></div></div></article>
<article class="glass card"><div class="card-head"><div style="display:flex;gap:12px;min-width:0"><span class="panel-brand-icon card-icon" aria-label="PasarGuard"><img src="/static/pasarguard-logo.png" alt="PasarGuard" onerror="this.style.display='none';this.parentElement.classList.add('logo-fallback')"><span class="logo-fallback-text">PG</span></span><div style="min-width:0"><h3 class="title">اطلاعات پنل</h3><p class="sub">اطلاعات پنل</p></div></div><span class="status-wrap">{status_badge('آنلاین' if panel_info['status'] == 'Online' else 'آفلاین', 'ok' if panel_info['status'] == 'Online' else 'bad')}</span></div>
...
<article class="glass card"><div class="card-head"><div style="display:flex;gap:12px">{ui_icon("telegram", "card-icon")}<div><h3 class="title">بکاپ تلگرام</h3><p class="sub">پشتیبان‌گیری تلگرام</p></div></div>{status_badge('فعال' if status == 'active' else 'متوقف', 'ok' if status == 'active' else 'bad')}</div>
<div class="meta"><div class="meta-row"><span>توکن ربات</span><span>{html.escape(masked)}</span></div><div class="meta-row"><span>شناسه گفتگو</span><span>{html.escape(c.get('chat') or 'تنظیم نشده')}</span></div><div class="meta-row"><span>شناسه تاپیک</span><span>{html.escape(c.get('topic') or '—')}</span></div><div class="meta-row"><span>بازه</span><span>{html.escape(str(c.get('interval','24')))} ساعت</span></div></div>
...
<article class="glass card backup-controls"><div class="card-head"><div style="display:flex;gap:12px;min-width:0">{ui_icon("settings", "card-icon")}<div style="min-width:0"><h3 class="title">تنظیمات Backup</h3><p class="sub">کنترل پشتیبان‌گیری</p></div></div>{status_badge("فعال" if status == "active" else "متوقف", "ok" if status == "active" else "bad")}</div>
...
<article class="glass wide"><div class="card-head"><div style="display:flex;gap:12px">{ui_icon("chart", "card-icon")}<div><h3 class="title">منابع سرور مجازی</h3><p class="sub">پردازنده · رم · دیسک · پایش لحظه‌ای</p></div></div></div>{_resource_chart_html()}</article>
        ...
        if path == "/backups":
            ...
            body=f"""<section class="hero"><h2>{ui_icon("backup", "hero-icon")} <span class="gradient">مدیریت پشتیبان‌ها</span></h2><p>پشتیبان‌های موجود را مشاهده، دانلود یا حذف کنید.</p></section><div class="glass wide"><div class="backup-list">{''.join(rows) or '<div class="empty">هنوز پشتیبانی پیدا نشد.</div>'}</div><div class="actions"><a class="btn" href="/">← داشبورد</a><form method="post" action="/backup">{hidden_csrf(self.sid())}<button class="btn primary" type="submit">{ui_icon("rocket", "inline-icon")} ساخت پشتیبان جدید</button></form></div></div>"""
            self.send_html(page("مدیریت پشتیبان‌ها",body)); return
        ...
        if path == "/backup-download":
            ...
            if not target or not target.is_file():
                self.send_html(page("یافت نشد",'<div class="glass"><div class="notice bad">پشتیبان پیدا نشد.</div></div>'),404); return
            try:
                ...
                self.send_html(page("پشتیبان‌گیری",'<div class="glass"><div class="notice bad">خواندن پشتیبان ناموفق بود.</div></div>'),500)
            return
        ...
        if path == "/account":
            ...
            self.send_html(page("حساب کاربری", body)); return

        if path == "/telegram":
            body = f'''<section class="hero"><h2>{ui_icon("telegram", "hero-icon")} <span class="gradient">بکاپ تلگرام</span></h2><p>اطلاعات ربات، مقصد، Topic، پروکسی و زمان‌بندی را تنظیم کنید؛ سپس Scheduler را شروع کنید.</p></section><div class="glass wide"><form method="post" action="/telegram">{hidden_csrf(self.sid())}<div class="grid"><div class="field" style="grid-column:span 6"><label>توکن ربات تلگرام</label><input name="token" value="{html.escape(c.get('token',''))}" placeholder="123456:ABC..." required><div class="hint">توکن BotFather را وارد کنید.</div></div><div class="field" style="grid-column:span 6"><label>شناسه گفتگو</label><input name="chat" value="{html.escape(c.get('chat',''))}" placeholder="-1001234567890" required></div><div class="field" style="grid-column:span 6"><label>شناسه تاپیک / رشته</label><input name="topic" value="{html.escape(c.get('topic',''))}" placeholder="12345"><div class="hint">شماره Topic را وارد کنید؛ لینک Topic تلگرام هم قابل قبول است.</div></div><div class="field" style="grid-column:span 6"><label>پروکسی تلگرام</label><input name="proxy" value="{html.escape(c.get('proxy',''))}" placeholder="socks5://127.0.0.1:1080"><div class="hint">اختیاری. اگر Proxy ندارید خالی بگذارید.</div></div></div><div class="actions"><button class="btn primary" type="submit">{ui_icon("settings", "inline-icon")} ذخیره تنظیمات</button><a class="btn" href="/test">{ui_icon("test", "inline-icon")} تست اتصال</a><a class="btn" href="/">← برگشت</a></div></form></div>'''
            self.send_html(page("پشتیبان‌گیری تلگرام", body)); return

        if path == "/backup-settings":
            checked = "checked" if c.get("node") else ""
            body = f'''<section class="hero"><h2>{ui_icon("settings", "hero-icon")} تنظیمات <span class="gradient">پشتیبان‌گیری</span></h2><p>کنترل زمان‌بندی و اجرای پشتیبان‌گیری دستی.</p></section><div class="grid"><article class="glass card"><div class="card-head"><div style="display:flex;gap:12px">{ui_icon("clock", "card-icon")}<div><h3 class="title">زمان‌بندی خودکار</h3><p class="sub">زمان‌بندی</p></div></div><span class="status {'on' if status=='active' else 'off'}">{html.escape(status)}</span></div><form method="post" action="/backup-settings">{hidden_csrf(self.sid())}<div class="field"><label>بازه پشتیبان‌گیری (ساعت)</label><input name="interval" type="number" step="0.5" min="0.5" max="720" value="{html.escape(str(c.get('interval','24')))}" required></div><label class="toggle"><span>شامل PG-Node شود</span><input type="checkbox" name="node" {checked}></label><div class="actions"><button class="btn primary" type="submit">{ui_icon("rocket", "inline-icon")} ذخیره و شروع</button><button class="btn danger" type="submit" formaction="/stop" formmethod="post">{ui_icon("activity", "inline-icon")} توقف</button></div></form></article><article class="glass card"><div class="card-head"><div style="display:flex;gap:12px">{ui_icon("backup", "card-icon")}<div><h3 class="title">پشتیبان‌گیری دستی</h3><p class="sub">پشتیبان‌گیری دستی</p></div></div></div><p class="empty">همین حالا یک پشتیبان کامل بگیرید و طبق تنظیمات تلگرام برای مقصد فعلی ارسال کنید.</p><form method="post" action="/backup">{hidden_csrf(self.sid())}<button class="btn good full">{ui_icon("rocket", "inline-icon")} شروع پشتیبان‌گیری دستی</button></form></article></div>'''
            self.send_html(page("تنظیمات پشتیبان‌گیری", body)); return

        if path == "/test":
            body = f'''<section class="hero"><h2>{ui_icon("test", "hero-icon")} تست <span class="gradient">تلگرام</span></h2><p>یک پیام آزمایشی با تنظیمات فعلی ارسال می‌شود.</p></section><div class="glass wide"><div class="meta"><div class="meta-row"><span>شناسه گفتگو</span><span>{html.escape(c.get('chat') or 'تنظیم نشده')}</span></div><div class="meta-row"><span>شناسه تاپیک</span><span>{html.escape(c.get('topic') or '—')}</span></div><div class="meta-row"><span>پروکسی</span><span>{html.escape(c.get('proxy') or 'بدون پروکسی')}</span></div></div><form method="post" action="/test">{hidden_csrf(self.sid())}<div class="actions"><button class="btn primary">{ui_icon("test", "inline-icon")} ارسال پیام تست</button><a class="btn" href="/">← برگشت</a></div></form></div>'''
            self.send_html(page("تست تلگرام", body)); return
    ...
    def do_POST(self):
        ...
        if path == "/setup":
            ...
            if c.get("password_hash"):
                self.send_html(page("راه‌اندازی", '<div class="glass"><div class="notice bad">راه‌اندازی اولیه قبلاً انجام شده است.</div><a class="btn" href="/login">ورود به پنل</a></div>', False), 403); return
            username = data.get("username", "").strip()
            pw = data.get("password", "")
            confirm = data.get("password_confirm", "")
            if not valid_username(username):
                self.send_html(page("راه‌اندازی", '<section class="login"><div class="glass"><div class="notice bad">نام کاربری باید ۵ تا ۳۲ کاراکتر و فقط شامل حروف انگلیسی، عدد یا خط تیره باشد.</div><a class="btn" href="/">تلاش دوباره</a></div></section>', False)); return
            if not valid_password(pw):
                self.send_html(page("راه‌اندازی", '<section class="login"><div class="glass"><div class="notice bad">رمز باید حداقل ۸ کاراکتر، شامل حداقل ۲ حرف، ۱ عدد و یکی از # @ * باشد.</div><a class="btn" href="/">تلاش دوباره</a></div></section>', False)); return
            if pw != confirm:
                self.send_html(page("راه‌اندازی", '<section class="login"><div class="glass"><div class="notice bad">تکرار رمز عبور با رمز جدید یکسان نیست.</div><a class="btn" href="/">تلاش دوباره</a></div></section>', False)); return
        ...
        if path == ADMIN_PATH:
            ...
            if not self.require_csrf(data):
                self.send_html(page("امنیت",'<div class="glass"><div class="notice bad">درخواست نامعتبر است.</div></div>'),403); return
            try:
                ...
                ui["brand_subtitle"]=str(data.get("brand_subtitle",ui.get("brand_subtitle","مرکز کنترل پشتیبان‌گیری · durwinam")))[:120]
                idont_save_ui_settings(ui)
                self.send_html(page("مدیریت",'<div class="glass"><div class="notice ok">تنظیمات با موفقیت ذخیره شد و روی کل وب‌پنل اعمال شد.</div><a class="btn primary" href="'+ADMIN_PATH+'">بازگشت به مدیریت</a></div>'))
            except Exception as exc:
                self.send_html(page("مدیریت",f'<div class="glass"><div class="notice bad">ذخیره تنظیمات ناموفق بود: {html.escape(str(exc))}</div><a class="btn" href="{ADMIN_PATH}">تلاش دوباره</a></div>'),500)
            return

        if not self.logged():
            self.redirect("/login"); return
        if not self.require_csrf(data):
            self.send_html(page("امنیت", '<div class="glass"><div class="notice bad">درخواست نامعتبر یا منقضی شده است. صفحه را دوباره باز کنید.</div></div>'), 403); return
        ...
        if path == "/account":
            ...
            confirm = data.get("password_confirm", "")
            if not valid_username(username):
                self.send_html(page("حساب کاربری", '<div class="glass"><div class="notice bad">نام کاربری باید ۵ تا ۳۲ کاراکتر و فقط شامل حروف انگلیسی، عدد یا خط تیره باشد.</div><a class="btn" href="/account">تلاش دوباره</a></div>'), 400); return
            c["username"] = username
            if pw or confirm:
                if not valid_password(pw):
                    self.send_html(page("حساب کاربری", '<div class="glass"><div class="notice bad">رمز جدید باید حداقل ۸ کاراکتر، شامل حروف انگلیسی، حداقل یک حرف بزرگ، یک عدد و یک کاراکتر ویژه باشد.</div><a class="btn" href="/account">تلاش دوباره</a></div>'), 400); return
                if pw != confirm:
                    self.send_html(page("حساب کاربری", '<div class="glass"><div class="notice bad">تکرار رمز جدید با رمز عبور یکسان نیست.</div><a class="btn" href="/account">تلاش دوباره</a></div>'), 400); return
                salt, digest = hash_password(pw)
                c.update({"password_salt": salt, "password_hash": digest})
            save_cfg(c)
            self.send_html(page("حساب کاربری", '<div class="glass"><div class="notice ok">تنظیمات حساب با موفقیت ذخیره شد.</div><div class="actions"><a class="btn primary" href="/">داشبورد</a><a class="btn" href="/account">حساب کاربری</a></div></div>')); return


        if path == "/telegram":
            c.update({"token": data.get("token", "").strip(), "chat": data.get("chat", "").strip(), "topic": data.get("topic", "").strip(), "proxy": data.get("proxy", "").strip()})
            save_cfg(c)
            self.send_html(page("تلگرام", '<div class="glass"><div class="notice ok">تنظیمات Telegram با موفقیت ذخیره شد.</div><a class="btn" href="/telegram">برگشت به Telegram</a></div>')); return
        ...
        if path == "/test":
            ...
            self.send_html(page("تست تلگرام", body, notice="", kind=kind)); return
        ...
        if path == "/backup-settings":
            ...
            ok = p.returncode == 0
            self.send_html(page("تنظیمات پشتیبان‌گیری", f'<div class="glass"><div class="notice {"ok" if ok else "bad"}">{"Scheduler ذخیره و شروع شد." if ok else "Scheduler ذخیره شد ولی شروع آن با خطا مواجه شد."}</div><a class="btn" href="/backup-settings">برگشت</a></div>')); return
        ...
        if path == "/backup-delete":
            ...
            target=next((item for item,_,_ in _backup_archives() if item.name==requested),None)
            if not target or not target.is_file():
                self.send_html(page("پشتیبان‌گیری",'<div class="glass"><div class="notice bad">پشتیبان پیدا نشد.</div></div>'),404); return
            try:
                target.unlink(); _record_activity(f"Backup حذف شد: {requested}","ok"); self.redirect("/backups")
            except OSError as exc:
                self.send_html(page("پشتیبان‌گیری",f'<div class="glass"><div class="notice bad">حذف پشتیبان ناموفق بود: {html.escape(str(exc))}</div></div>'),500)
            return
        ...
        if path == "/backup":
            try:
                ...
                ok, msg = False, str(e)
            body = f'<div class="glass"><div class="notice {"ok" if ok else "bad"}">{status_dot("ok" if ok else "bad")} {("Backup با موفقیت ساخته و ارسال شد." if ok else "پشتیبان‌گیری ناموفق بود.")}<br><span class="empty">{html.escape(str(msg))}</span></div><div class="actions"><a class="btn" href="/backup-settings">تنظیمات Backup</a><a class="btn" href="/">داشبورد</a></div></div>'
            self.send_html(page("پشتیبان‌گیری دستی", body)); return
...
def main():
    ...
    if args.worker:
        worker(); return
    print(f"{APP} وب‌پنل v{VERSION} listening on http://{HOST}:{PORT}")
