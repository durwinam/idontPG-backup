...
.language-picker{position:relative}.language-toggle{min-width:46px;width:46px;height:42px;padding:0;display:grid;place-items:center;border:1px solid var(--line);border-radius:14px;color:#67e8f9;background:linear-gradient(145deg,rgba(34,211,238,.10),rgba(139,92,246,.12));backdrop-filter:blur(18px) saturate(145%);-webkit-backdrop-filter:blur(18px) saturate(145%);cursor:pointer;transition:transform .25s ease,box-shadow .25s ease,border-color .25s ease,color .25s ease}.language-toggle svg{width:21px;height:21px;fill:none;stroke:currentColor;stroke-width:1.65;stroke-linecap:round;stroke-linejoin:round;filter:drop-shadow(0 0 3px currentColor) drop-shadow(0 0 8px currentColor)}.language-toggle:hover,.language-toggle[aria-expanded="true"]{transform:translateY(-2px);border-color:rgba(34,211,238,.58);box-shadow:0 0 18px rgba(34,211,238,.18),0 10px 30px rgba(34,211,238,.12)}.language-toggle[aria-expanded="true"] svg{transform:rotate(12deg);transition:transform .25s ease}.language-menu{position:absolute;top:50px;right:0;z-index:131;min-width:155px;padding:7px;border:1px solid var(--line);border-radius:16px;background:rgba(12,14,24,.94);backdrop-filter:blur(22px) saturate(150%);-webkit-backdrop-filter:blur(22px) saturate(150%);box-shadow:0 18px 50px rgba(0,0,0,.35)}.language-menu[hidden]{display:none}.language-menu button{width:100%;display:flex;align-items:center;gap:9px;padding:10px 11px;border:0;border-radius:11px;background:transparent;color:var(--text);cursor:pointer;font:inherit;text-align:right}.language-menu button:hover,.language-menu button.active{background:rgba(124,58,237,.12)}body.light .language-menu{background:rgba(255,255,255,.96);border-color:rgba(124,58,237,.18);box-shadow:0 18px 50px rgba(124,58,237,.18)}body.light .language-menu button{color:#26162d}body.light .language-menu button:hover,body.light .language-menu button.active{background:rgba(124,58,237,.08)}.language-menu .lang-flag{display:inline-grid;place-items:center;min-width:28px;height:20px;padding:0 5px;border-radius:7px;font-size:9px;font-weight:900;letter-spacing:.4px;color:currentColor;border:1px solid currentColor;opacity:.8}.language-menu button{justify-content:flex-start}.light .language-toggle{color:#6d28d9;background:linear-gradient(145deg,rgba(255,255,255,.90),rgba(237,233,254,.72));border-color:rgba(109,40,217,.20);box-shadow:0 8px 24px rgba(109,40,217,.10),0 0 16px rgba(236,72,153,.06)}.light .language-toggle:hover,.light .language-toggle[aria-expanded="true"]{border-color:rgba(109,40,217,.42);box-shadow:0 0 18px rgba(109,40,217,.16),0 10px 30px rgba(109,40,217,.10)}
...
def ui_language_menu():
    return """<div class="language-picker"><button class="language-toggle" id="languageToggle" type="button" aria-label="تغییر زبان" title="Language" aria-haspopup="menu" aria-expanded="false"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="8.8"/><path d="M3.5 12h17M12 3.2c2.2 2.4 3.4 5.3 3.4 8.8S14.2 18.4 12 20.8M12 3.2C9.8 5.6 8.6 8.5 8.6 12s1.2 6.4 3.4 8.8"/></svg></button><div class="language-menu" id="languageMenu" role="menu" hidden><button type="button" role="menuitem" data-lang-choice="fa"><span class="lang-flag">IR</span><span>فارسی</span></button><button type="button" role="menuitem" data-lang-choice="en"><span class="lang-flag">EN</span><span>English</span></button><button type="button" role="menuitem" data-lang-choice="ru"><span class="lang-flag">RU</span><span>Русский</span></button></div></div>"""
...
function setLanguage(lang){
  ...
  const btn=document.getElementById("languageToggle"); if(btn){ btn.title=lang==="fa"?"زبان":lang==="ru"?"Язык":"Language"; btn.setAttribute("aria-expanded","false"); }
...
if(!maps[current]) current="fa";
if(lb) lb.addEventListener("click",e=>{e.stopPropagation();if(lm){lm.hidden=!lm.hidden;lb.setAttribute("aria-expanded",String(!lm.hidden));}});
if(lm) lm.querySelectorAll("[data-lang-choice]").forEach(x=>x.addEventListener("click",()=>{setLanguage(x.dataset.langChoice);if(lb)lb.setAttribute("aria-expanded","false")}));
document.addEventListener("click",e=>{if(lm&&!lm.hidden&&!lm.contains(e.target)&&e.target!==lb){lm.hidden=true;if(lb)lb.setAttribute("aria-expanded","false")}});
setLanguage(current);
})();"""
