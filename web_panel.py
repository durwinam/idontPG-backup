...
(function(){
  ...
  function setVals(u){['cpu','ram','disk'].forEach(k=>document.getElementById(k+'Value').textContent=Number(u[k]).toFixed(1)+'%');history.push({cpu:Number(u.cpu),ram:Number(u.ram),disk:Number(u.disk)});history.shift();render();const lang=document.documentElement.lang||'en';const labels={fa:'به‌روزرسانی: ',en:'Updated: ',ru:'Обновлено: '};document.getElementById('resourceUpdated').textContent=(labels[lang]||labels.en)+new Date().toLocaleTimeString(lang==='fa'?'fa-IR':lang==='ru'?'ru-RU':'en-US',{hour:'2-digit',minute:'2-digit',second:'2-digit'});}
  render();
...
.language-picker{position:relative}.language-toggle{display:grid;place-items:center}.language-svg{width:22px;height:22px;fill:none;stroke:currentColor;stroke-width:1.55;stroke-linecap:round;stroke-linejoin:round;filter:drop-shadow(0 0 3px currentColor) drop-shadow(0 0 9px currentColor);transition:.25s ease}.language-toggle:hover .language-svg{transform:rotate(12deg) scale(1.08);filter:drop-shadow(0 0 4px currentColor) drop-shadow(0 0 13px currentColor)}.lang-flag{display:inline-grid;place-items:center;min-width:30px;height:22px;padding:0 6px;border-radius:7px;background:linear-gradient(135deg,rgba(124,58,237,.18),rgba(34,211,238,.13));border:1px solid var(--line);font-size:9px;font-weight:900;letter-spacing:.5px}.language-menu button{direction:ltr}.language-menu button span:last-child{flex:1}.light .language-svg{filter:drop-shadow(0 0 2px rgba(109,40,217,.25))}.custom .language-svg{filter:drop-shadow(0 0 3px var(--accent2)) drop-shadow(0 0 9px var(--accent2))}.language-menu{position:absolute;top:50px;right:0;z-index:131;min-width:155px;padding:7px;border:1px solid var(--line);border-radius:16px;background:rgba(12,14,24,.94);backdrop-filter:blur(22px) saturate(150%);-webkit-backdrop-filter:blur(22px) saturate(150%);box-shadow:0 18px 50px rgba(0,0,0,.35)}.language-menu[hidden]{display:none}.language-menu button{width:100%;display:flex;align-items:center;gap:9px;padding:10px 11px;border:0;border-radius:11px;background:transparent;color:var(--text);cursor:pointer;font:inherit;text-align:right}.language-menu button:hover,.language-menu button.active{background:rgba(124,58,237,.12)}body.light .language-menu{background:rgba(255,255,255,.96);border-color:rgba(124,58,237,.18);box-shadow:0 18px 50px rgba(124,58,237,.18)}body.light .language-menu button{color:#26162d}body.light .language-menu button:hover,body.light .language-menu button.active{background:rgba(124,58,237,.08)}
...
def ui_language_menu():
    return '''<div class="language-picker"><button class="language-toggle" id="languageToggle" type="button" aria-label="تغییر زبان" title="تغییر زبان"><svg class="language-svg" viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="8.6"/><path d="M3.4 12h17.2M12 3.4c2.25 2.35 3.35 5.2 3.35 8.6S14.25 18.25 12 20.6c-2.25-2.35-3.35-5.2-3.35-8.6S9.75 5.75 12 3.4Z"/></svg></button><div class="language-menu" id="languageMenu" hidden><button type="button" data-lang-choice="fa"><span class="lang-flag">IR</span><span>فارسی</span></button><button type="button" data-lang-choice="en"><span class="lang-flag">EN</span><span>English</span></button><button type="button" data-lang-choice="ru"><span class="lang-flag">RU</span><span>Русский</span></button></div></div>'''
...
const textSnapshot=new WeakMap(), attrSnapshot=new WeakMap();
function snapshot(){
  document.querySelectorAll("body *:not(script):not(style)").forEach(el=>{
    ["title","aria-label","placeholder","value"].forEach(a=>{
      if(el.hasAttribute(a)){let m=attrSnapshot.get(el);if(!m){m={};attrSnapshot.set(el,m)}if(!(a in m))m[a]=el.getAttribute(a)}
    });
    const walker=document.createTreeWalker(el,NodeFilter.SHOW_TEXT);
    let n; while(n=walker.nextNode()){if(!textSnapshot.has(n))textSnapshot.set(n,n.nodeValue);}
  });
}
function translateString(raw,dict){
  if(!raw)return raw; let out=raw;
...
function setLanguage(lang){
  ...
  document.querySelectorAll("body *:not(script):not(style)").forEach(el=>{
    const am=attrSnapshot.get(el);
    if(am) Object.keys(am).forEach(a=>el.setAttribute(a,lang==="fa"?am[a]:translateString(am[a],dict)));
  });
  const walker=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT);
  let n; while(n=walker.nextNode()){
    if(textSnapshot.has(n)){const original=textSnapshot.get(n);n.nodeValue=lang==="fa"?original:translateString(original,dict);}
  }
  const lm=document.getElementById("languageMenu"); if(lm) lm.hidden=true;
  const lb=document.getElementById("languageToggle"); if(lb) lb.title=lang==="fa"?"تغییر زبان":lang==="ru"?"Изменить язык":"Change language";
  document.querySelectorAll("[data-lang-choice]").forEach(x=>x.classList.toggle("active",x.dataset.langChoice===lang));
  const u=document.getElementById("resourceUpdated");
  if(u){const now=new Date();u.textContent=(lang==="fa"?"به‌روزرسانی: ":lang==="ru"?"Обновлено: ":"Updated: ")+now.toLocaleTimeString(lang==="fa"?"fa-IR":lang==="ru"?"ru-RU":"en-US",{hour:"2-digit",minute:"2-digit",second:"2-digit"});}
