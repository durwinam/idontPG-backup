...
SESSION_BLOCK_FILE = STATE_DIR / "blocked_sessions.json"

def _load_blocked_sessions():
    try:
        if SESSION_BLOCK_FILE.is_file():
            raw=json.loads(SESSION_BLOCK_FILE.read_text(encoding="utf-8"))
            return raw if isinstance(raw, list) else []
    except Exception:
        pass
    return []

def _save_blocked_sessions(items):
    try:
        STATE_DIR.mkdir(parents=True, exist_ok=True)
        tmp=SESSION_BLOCK_FILE.with_suffix(".tmp")
        tmp.write_text(json.dumps(items[-300:], ensure_ascii=False), encoding="utf-8")
        os.chmod(tmp,0o600); tmp.replace(SESSION_BLOCK_FILE)
    except Exception:
        pass

def _session_is_blocked(ip, device):
    ip=str(ip or "unknown"); device=str(device or "Unknown")
    for item in _load_blocked_sessions():
        if item.get("ip") == ip or (item.get("device") and item.get("device") == device and item.get("match_device")):
            return True
    return False

def _block_session_target(ip, device, username="admin"):
    items=_load_blocked_sessions()
    entry={"ip":str(ip or "unknown"),"device":str(device or "Unknown"),"match_device":True,"username":str(username or "admin"),"time":time.time()}
    items=[x for x in items if not (x.get("ip")==entry["ip"] and x.get("device")==entry["device"])]
...
_VERSION_CACHE={"ts":0,"version":""}
def get_remote_version():
    now=time.time()
    if now-_VERSION_CACHE["ts"]<600: return _VERSION_CACHE["version"]
    try:
        req=urllib.request.Request("https://raw.githubusercontent.com/durwinam/idontPG-backup/main/pg_backup.py",headers={"User-Agent":f"{APP}/{VERSION}"})
        with urllib.request.urlopen(req,timeout=4) as r: text=r.read().decode("utf-8","ignore")
        m=re.search(r'VERSION\s*=\s*["\']([^"\']+)["\']',text)
        if m: _VERSION_CACHE.update({"ts":now,"version":m.group(1)}); return m.group(1)
...
def telegram_send(token, chat, topic, file_path, caption="", proxy=None):
    ...
    try:
        with proxy_opener(proxy).open(req, timeout=300) as r:
            payload = json.loads(r.read().decode("utf-8"))
            if payload.get("ok") and payload.get("result", {}).get("message_id") and load_cfg().get("telegram_auto_delete"):
                cfg=load_cfg(); hours=float(cfg.get("telegram_auto_delete_hours") or 0)
                if 0.5 <= hours <= 48:
                    jobs=list(cfg.get("telegram_delete_jobs") or [])
                    jobs.append({"chat_id":str(chat),"message_id":int(payload["result"]["message_id"]),"sent_at":time.time(),"delete_at":time.time()+hours*3600})
                    cfg["telegram_delete_jobs"]=jobs[-500:]; save_cfg(cfg)
            return bool(payload.get("ok")), payload.get("description", "OK")
    except Exception as e:
        return False, str(e)
...
def make_backup(send=True, audit_context=None):
    c = load_cfg()
    core = load_core()
    audit_context = audit_context or {}
    au, ai, ad = audit_context.get("username","system"), audit_context.get("ip","unknown"), audit_context.get("device","Unknown")
    try:
        _record_activity("Backup started", "ok", au, ai, ad, "backup_start")
        ...
        ok, msg = send_archive(core, archive, c, f"PasarGuard + PG-Node Auto Backup\nDate: {time.strftime('%Y-%m-%d %H:%M:%S')}\nInterval: {c.get('interval', '24')}h\ndurwinam")
        _record_activity("Backup sent to Telegram successfully" if ok else "Backup created but Telegram send failed", "ok" if ok else "bad", au, ai, ad, "telegram_send_success" if ok else "telegram_send_failed")
        return ok, msg
    except Exception as exc:
        _record_activity("Backup failed", "bad", au, ai, ad, "backup_failed")
        raise
...
def _panel_read_env():
    """Read PasarGuard's .env using the same source path as idontPG-backup."""
...
def get_panel_info():
    """Build/check the PasarGuard panel URL using idontPG-backup's access model."""
...
AUDIT_LOG_FILE = STATE_DIR / "audit_logs.json"
...
def _record_audit(message, kind="ok", username="system", ip="unknown", device="Unknown", event="activity"):
    ...
    try:
        ...
        if AUDIT_LOG_FILE.is_file():
            try:
                raw = json.loads(AUDIT_LOG_FILE.read_text(encoding="utf-8"))
                items = raw if isinstance(raw, list) else []
            except Exception:
                items = []
        items.insert(0, {"time": time.time(), "message": str(message), "kind": str(kind),
                         "event": str(event), "username": str(username or "system"),
                         "ip": str(ip or "unknown"), "device": str(device or "Unknown")})
        AUDIT_LOG_FILE.write_text(json.dumps(items[:200], ensure_ascii=False), encoding="utf-8")
        try: AUDIT_LOG_FILE.chmod(0o600)
        except OSError: pass
    except Exception:
        pass

def _record_activity(message, kind="ok", username="system", ip="unknown", device="Unknown", event="activity"):
    """Keep the dashboard's 3 recent activities and also write the full audit log."""
    _record_audit(message, kind, username, ip, device, event)
    try:
        STATE_DIR.mkdir(parents=True, exist_ok=True)
        items = []
...
def _login_device(user_agent):
    ua=str(user_agent or "").lower()
    if "android" in ua: platform="Android"
    elif "iphone" in ua or "ipad" in ua or "ios" in ua: platform="iOS"
...
def _record_login(ip, username, role="user", device="Unknown"):
    try:
        STATE_DIR.mkdir(parents=True, exist_ok=True)
        now=time.time(); items=[]
        if LOGIN_LOG_FILE.is_file():
            try:
                raw=json.loads(LOGIN_LOG_FILE.read_text(encoding="utf-8")); items=raw if isinstance(raw,list) else []
            except Exception: items=[]
        items.insert(0,{"time":now,"ip":str(ip or "unknown"),"username":str(username or "admin"),"role":role,"device":str(device or "Unknown")})
        LOGIN_LOG_FILE.write_text(json.dumps(items[:100],ensure_ascii=False),encoding="utf-8")
        try: LOGIN_LOG_FILE.chmod(0o600)
        except OSError: pass
        return now
...
def get_login_logs():
    try:
        if LOGIN_LOG_FILE.is_file():
            raw=json.loads(LOGIN_LOG_FILE.read_text(encoding="utf-8")); return raw if isinstance(raw,list) else []
    except Exception: pass
    return []
...
_UI_ICONS = {
    ...
    "upload": '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 16V4"/><path d="m7 9 5-5 5 5"/><path d="M5 13v5a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-5"/></svg>',
    "database": '<svg viewBox="0 0 24 24" aria-hidden="true"><ellipse cx="12" cy="5.5" rx="7.5" ry="3"/><path d="M4.5 5.5v6c0 1.7 3.4 3 7.5 3s7.5-1.3 7.5-3v-6"/><path d="M4.5 11.5v6c0 1.7 3.4 3 7.5 3s7.5-1.3 7.5-3v-6"/></svg>',
    "shield": '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3 19 6v5.5c0 4.5-2.8 7.5-7 9.5-4.2-2-7-5-7-9.5V6l7-3Z"/><path d="m9 12 2 2 4-4"/></svg>',
    "terminal": '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m5 7 5 5-5 5"/><path d="M12 17h7"/></svg>',
    ...
    <div class="plot-body">
      ...
      <svg class="telemetry" id="telemetryChart" viewBox="0 0 900 300" preserveAspectRatio="none" role="img" aria-label="نمودار لحظه‌ای منابع سرور">
        <defs>
          ...
          <filter id="glowC" x="-30%" y="-80%" width="160%" height="260%"><feGaussianBlur stdDeviation="5.5" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
          <filter id="glowP" x="-30%" y="-80%" width="160%" height="260%"><feGaussianBlur stdDeviation="5" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
        </defs>
...
(function(){
  ...
  function coords(values){
    const n=values.length,w=900,h=300;
    return values.map((v,i)=>({x:i*(w/(n-1)),y:h-(clamp(v)/100)*h}));
  }
  function smoothPath(values){
    const pts=coords(values); if(!pts.length) return '';
  ...
  function area(values){
    const pts=coords(values); if(!pts.length) return '';
    return smoothPath(values)+' L 900 300 L 0 300 Z';
  }
  function point(values){
    const pts=coords(values), p=pts[pts.length-1]||{x:900,y:300};
    return [p.x,p.y];
  }
  function render(){
    ['cpu','ram','disk'].forEach(k=>{
      const vals=history.map(x=>x[k]);
      document.getElementById(k+'Line').setAttribute('d',smoothPath(vals));
      document.getElementById(k+'Area').setAttribute('d',area(vals));
      const pt=point(vals),dot=document.getElementById(k+'Dot');
...
.neo-icon{position:relative;display:inline-grid;place-items:center;flex:0 0 44px;width:44px;height:44px;border-radius:15px;color:#67e8f9;background:linear-gradient(145deg,rgba(103,232,249,.16),rgba(124,58,237,.20) 58%,rgba(236,72,153,.12));border:1px solid rgba(103,232,249,.42);box-shadow:inset 0 1px 0 rgba(255,255,255,.11),0 0 10px rgba(103,232,249,.22),0 0 30px rgba(34,211,238,.16),0 10px 30px rgba(124,58,237,.12);transition:transform .25s ease,box-shadow .25s ease,color .25s ease,border-color .25s ease;overflow:visible}
.neo-icon:before{content:"";position:absolute;inset:5px;border-radius:11px;background:radial-gradient(circle,rgba(255,255,255,.07),transparent 68%);pointer-events:none}.neo-icon:after{display:none!important}.neo-icon svg{position:relative;z-index:1;width:22px;height:22px;fill:none;stroke:currentColor;stroke-width:1.75;stroke-linecap:round;stroke-linejoin:round;filter:drop-shadow(0 0 3px currentColor) drop-shadow(0 0 8px currentColor)}@keyframes iconSpark{0%,100%{transform:scale(.72);opacity:.45}50%{transform:scale(1.18);opacity:1}}
...
.neo-icon.meta-icon,.neo-icon.inline-icon{border:0!important;background:transparent!important;box-shadow:none!important;outline:0!important;padding:0!important}
.neo-icon.meta-icon:focus,.neo-icon.inline-icon:focus{outline:0!important;box-shadow:none!important}
.neo-icon:hover{transform:translateY(-2px) scale(1.035);box-shadow:inset 0 1px 0 rgba(255,255,255,.13),0 0 12px rgba(103,232,249,.34),0 0 38px rgba(34,211,238,.24),0 12px 34px rgba(124,58,237,.16)}
/* v5.8.0: light theme icon polish — keep SVG icons readable and visually matched to the light glass UI. */
...
/* v5.8.0 — Clean Glass UI
   Unified, crisp glow system: soft edges, restrained bloom, readable controls. */
:root{
  --clean-glass-shadow:0 18px 52px rgba(0,0,0,.30);
  --clean-glass-inset:inset 0 1px 0 rgba(255,255,255,.075);
}
.glass{
  background:linear-gradient(145deg,rgba(18,21,36,.66),rgba(255,255,255,.025)) !important;
  box-shadow:var(--clean-glass-shadow),var(--clean-glass-inset) !important;
...
.card:hover{
  ...
  box-shadow:0 22px 64px rgba(0,0,0,.34),0 0 20px rgba(103,232,249,.055),var(--clean-glass-inset) !important;
}
.neo-icon{
  background:linear-gradient(145deg,rgba(103,232,249,.115),rgba(124,58,237,.145) 60%,rgba(236,72,153,.075)) !important;
  border-color:rgba(103,232,249,.30) !important;
  box-shadow:var(--clean-glass-inset),0 0 9px rgba(103,232,249,.13),0 8px 22px rgba(124,58,237,.08) !important;
}
.neo-icon svg{
  filter:drop-shadow(0 0 2px currentColor) drop-shadow(0 0 4px color-mix(in srgb,currentColor 35%,transparent)) !important;
}
.neo-icon:hover{
  transform:translateY(-1px) scale(1.02);
  box-shadow:var(--clean-glass-inset),0 0 12px rgba(103,232,249,.19),0 10px 26px rgba(124,58,237,.10) !important;
}
.neo-icon.card-icon{
  box-shadow:var(--clean-glass-inset),0 0 11px rgba(103,232,249,.14),0 10px 28px rgba(124,58,237,.09) !important;
}
.neo-icon.card-icon svg{filter:drop-shadow(0 0 2px currentColor) drop-shadow(0 0 5px color-mix(in srgb,currentColor 30%,transparent)) !important}
.neo-icon.meta-icon,.neo-icon.inline-icon{box-shadow:none !important}
.neo-icon.meta-icon svg{filter:drop-shadow(0 0 2px currentColor) !important}

.hero-icon .neo-icon{
  box-shadow:var(--clean-glass-inset),0 0 12px rgba(103,232,249,.16),0 12px 30px rgba(34,211,238,.08) !important;
}
...
/* Destination database — clean glass dropdown. */
.glass-select-trigger{
  ...
  box-shadow:var(--clean-glass-inset),0 8px 24px rgba(0,0,0,.10) !important;
  backdrop-filter:blur(18px) saturate(125%);
  -webkit-backdrop-filter:blur(18px) saturate(125%);
}
.glass-select-trigger:hover,.glass-select.open .glass-select-trigger{
  background:linear-gradient(145deg,rgba(103,232,249,.095),rgba(255,255,255,.035)) !important;
  border-color:rgba(103,232,249,.40) !important;
  box-shadow:var(--clean-glass-inset),0 0 11px rgba(103,232,249,.12),0 10px 28px rgba(0,0,0,.13) !important;
}
.glass-select-menu{
  background:linear-gradient(145deg,rgba(16,21,36,.94),rgba(9,13,25,.90)) !important;
  border-color:rgba(103,232,249,.24) !important;
  box-shadow:0 18px 46px rgba(0,0,0,.38),0 0 15px rgba(103,232,249,.08),var(--clean-glass-inset) !important;
  backdrop-filter:blur(22px) saturate(125%);
  -webkit-backdrop-filter:blur(22px) saturate(125%);
}
.glass-select-option{color:var(--text) !important;background:transparent !important}
.glass-select-option:hover{
  background:linear-gradient(90deg,rgba(124,58,237,.11),rgba(34,211,238,.085)) !important;
  border-color:rgba(103,232,249,.20) !important;
  box-shadow:none !important;
}
.glass-select-option.selected{
  background:linear-gradient(90deg,rgba(124,58,237,.15),rgba(34,211,238,.10)) !important;
  border-color:rgba(103,232,249,.24) !important;
}
.glass-select-option .check{text-shadow:0 0 6px rgba(103,232,249,.38) !important}

.btn.primary{
  box-shadow:0 8px 22px rgba(124,58,237,.22),0 0 13px rgba(236,72,153,.10) !important;
}
.btn.primary:hover{
  box-shadow:0 10px 26px rgba(124,58,237,.26),0 0 16px rgba(236,72,153,.12) !important;
}
.btn:hover{box-shadow:0 8px 22px rgba(124,58,237,.11) !important}

.toggle,.meta-row,.resource-stat,.theme-toggle,.menu-toggle{
  box-shadow:var(--clean-glass-inset),0 8px 24px rgba(0,0,0,.08) !important;
}
.toggle input:checked{box-shadow:0 0 10px rgba(52,211,153,.10),inset 0 1px 2px rgba(0,0,0,.20) !important}
.toggle input:checked::after{box-shadow:0 0 8px rgba(110,231,183,.48) !important}

.drawer{
  box-shadow:0 24px 78px rgba(0,0,0,.44),0 0 30px rgba(124,58,237,.08),var(--clean-glass-inset) !important;
}
.drawer-icon{box-shadow:var(--clean-glass-inset),0 0 9px rgba(124,58,237,.08) !important}
.menu-toggle:hover,.theme-toggle:hover{box-shadow:0 8px 24px rgba(124,58,237,.11) !important}

.status-dot{animation:cleanStatusBreath 1.8s ease-in-out infinite !important}
.status-dot.status-off{animation:none !important;box-shadow:none !important}
@keyframes cleanStatusBreath{
  0%,100%{opacity:.78;box-shadow:0 0 2px currentColor,0 0 5px color-mix(in srgb,currentColor 55%,transparent)}
  50%{opacity:1;box-shadow:0 0 3px currentColor,0 0 7px color-mix(in srgb,currentColor 62%,transparent)}
}

.light .glass{
  background:linear-gradient(145deg,rgba(255,255,255,.72),rgba(255,255,255,.43)) !important;
  box-shadow:0 18px 52px rgba(74,40,100,.11),inset 0 1px 0 rgba(255,255,255,.90) !important;
}
.light .neo-icon{
  box-shadow:inset 0 1px 0 rgba(255,255,255,.95),0 7px 20px rgba(109,40,217,.10),0 0 10px rgba(236,72,153,.055) !important;
}
.light .neo-icon svg{filter:drop-shadow(0 1px 2px rgba(109,40,217,.14)) !important}
.light .glass-select-menu{
  box-shadow:0 18px 46px rgba(31,20,60,.16),0 0 12px rgba(34,211,238,.055),inset 0 1px 0 rgba(255,255,255,.85) !important;
}
.light 

body.custom .glass{
  box-shadow:0 18px 52px var(--shadow-color),inset 0 1px 0 rgba(255,255,255,.065) !important;
}
body.custom .neo-icon{
  box-shadow:inset 0 1px 0 rgba(255,255,255,.075),0 0 10px color-mix(in srgb,var(--accent2) 13%,transparent),0 8px 22px color-mix(in srgb,var(--accent) 8%,transparent) !important;
}
body.custom .glass-select-menu{
  box-shadow:0 18px 46px var(--shadow-color),0 0 13px color-mix(in srgb,var(--accent2) 8%,transparent),inset 0 1px 0 rgba(255,255,255,.07) !important;
}
...
EN_TRANSLATIONS = {
       "بازگردانی داده‌های اختصاصی وب‌پنل.":"Restore Web Panel data.", "می‌خوای بکاپ بگیری یا برگردی؟ قبل از شروع سلامت Backup و مقصد بررسی می‌شود.":"Choose whether to create a backup or restore. Backup health and destination are checked first.", "فایل بکاپ (ZIP) را انتخاب یا اینجا رها کنید":"Select or drop a backup ZIP file here", "فقط فایل‌های ZIP مجاز هستند":"ZIP files only", "انتخاب فایل":"Choose file", "فایلی انتخاب نشده":"No file selected", "دستورهای نصب دیتابیس PasarGuard":"PasarGuard database installation commands", "پیشنهادی":"Recommended", "دستور مناسب دیتابیس مقصد را کپی کنید.":"Copy the installation command for your destination database.", "کپی شد":"Copied", "جستجوی فرمان‌ها…":"Search commands…", "تشخیص خطا":"Run Diagnostics", "مرکز امنیت":"Security Center", "مرکز اعلان‌ها":"Notification Center", "مدیریت نشست‌ها":"Session Management", "خط زمانی بکاپ":"Backup Timeline", "پالت فرمان":"Command Palette",   "Run Diagnostics":"Run Diagnostics", "Security Center":"Security Center", "Notification Center":"Notification Center", "Session Management":"Session Management", "Backup Timeline":"Backup Timeline", "PasarGuard روی مقصد شناسایی شد.":"PasarGuard detected on destination.", "Restore با موفقیت کامل شد.":"Restore completed successfully.", "Restore ناموفق بود":"Restore failed",
    ...
    "رمز جدید باید حداقل ۸ کاراکتر، شامل حروف انگلیسی، یک عدد و یک کاراکتر ویژه باشد.": "New password must be at least 8 characters and include letters, at least one uppercase letter, one number, and one special character.",
...
RU_TRANSLATIONS = {
    "Restore Center":"Центр восстановления", "Restore Web Panel":"Восстановление Web Panel", "Restore PasarGuard":"Восстановление PasarGuard", "بازگردانی داده‌های اختصاصی وب‌پنل.":"Данные Web Panel", "می‌خوای بکاپ بگیری یا برگردی؟ قبل از شروع سلامت Backup و مقصد بررسی می‌شود.":"Выберите резервное копирование или восстановление. Перед началом проверяются Backup и сервер назначения.", "فایل بکاپ (ZIP) را انتخاب یا اینجا رها کنید":"Выберите ZIP-файл Backup или перетащите его сюда", "فقط فایل‌های ZIP مجاز هستند":"Разрешены только ZIP-файлы", "انتخاب فایل":"Выбрать файл", "فایلی انتخاب نشده":"Файл не выбран", "دستورهای نصب دیتابیس PasarGuard":"Команды установки баз данных PasarGuard", "پیشنهادی":"Рекомендуется", "دستور مناسب دیتابیس مقصد را کپی کنید.":"Скопируйте команду установки для базы назначения.", "کپی شد":"Скопировано", "مرکز بازیابی":"Центр восстановления", "بازیابی وب‌پنل":"Восстановление Web Panel", "بازیابی PasarGuard":"Восстановление PasarGuard", "جستجوی فرمان‌ها…":"Поиск команд…", "تشخیص خطا":"Диагностика", "مرکز امنیت":"Центр безопасности", "مرکز اعلان‌ها":"Центр уведомлений", "مدیریت نشست‌ها":"Управление сессиями", "خط زمانی بکاپ":"Хронология Backup", "پالت فرمان":"Палитра команд", "PasarGuard روی سرور مقصد نصب نیست.":"PasarGuard не установлен на сервере назначения.", "پایگاه‌داده مقصد":"База данных назначения", "دیتابیس مقصد":"База данных назначения", "TimescaleDB (پیشنهادی)":"TimescaleDB (рекомендуется)", "تاریخچه ترافیک Nodeها حفظ شود":"Сохранить историю трафика Node", "Nodeها بعد از Restore غیرفعال بمانند":"Оставить Node отключёнными после восстановления", "قبل از Restore Backup ایمنی بگیر":"Создать резервную копию перед восстановлением", "Run Diagnostics":"Диагностика", "Security Center":"Центр безопасности", "Notification Center":"Центр уведомлений", "Session Management":"Управление сессиями", "Backup Timeline":"Хронология Backup", "فرآیند Restore":"Процесс восстановления", "در حال بررسی سلامت Backup…":"Проверка Backup…", "Backup سالم است":"Backup исправен", "PasarGuard روی مقصد شناسایی شد.":"PasarGuard обнаружен на сервере назначения", "Restore با موفقیت کامل شد.":"Восстановление успешно завершено", "Restore ناموفق بود":"Восстановление не удалось",
    # Core UI
...
def page(title, body, logged=True, notice="", kind="ok"):
    ...
    css = '''<style>
.public-language{position:fixed;top:18px;right:18px;z-index:999}.language-picker{position:relative;display:inline-flex}.language-toggle{width:46px;height:42px;border:1px solid var(--line);border-radius:14px;background:var(--glass2);color:var(--text);cursor:pointer;font-weight:900;font-size:17px;box-shadow:inset 0 1px 0 rgba(255,255,255,.08),0 0 16px rgba(34,211,238,.12)}.language-menu{position:absolute;right:0;top:50px;min-width:150px;padding:7px;border:1px solid var(--line);border-radius:16px;background:rgba(10,12,22,.96);backdrop-filter:blur(18px);z-index:100;box-shadow:0 18px 50px rgba(0,0,0,.35)}.language-menu a{display:flex;align-items:center;gap:9px;padding:10px 11px;border-radius:11px;color:var(--text);text-decoration:none;font-size:12px;font-weight:800}.language-menu a:hover{background:var(--glass2)}html[lang="en"]{direction:ltr}html[lang="en"] body{direction:ltr}html[lang="en"] .drawer{direction:ltr}html[lang="en"] .language-menu,html[lang="ru"] .language-menu{text-align:left;right:0;left:auto}html[lang="ru"]{direction:ltr}html[lang="ru"] body{direction:ltr}html[lang="ru"] .drawer{direction:ltr}html[lang="ru"] .language-menu{direction:ltr}
</style>'''
...
class Handler(BaseHTTPRequestHandler):
    ...
    def sid(self):
        ...
        device=_login_device(self.headers.get("User-Agent",""))
        if _session_is_blocked(ip,device):
            SESSIONS.pop(sid,None)
            return None
        return sid
    ...
    def do_GET(self):
        ...
        if path == "/language":
            ...
            if self.logged():
                ...
                _record_activity(f"Language changed to {lang}","ok",si.get("username","system"),self.client_address[0] if self.client_address else "unknown",_login_device(self.headers.get("User-Agent","")),"language_change")
            self.send_response(302)
        ...
        if path == "/logout":
            ...
            if sid:
                si=SESSIONS.get(sid,{})
                _record_activity("Panel logout","ok",si.get("username","system"),self.client_address[0] if self.client_address else "unknown",_login_device(self.headers.get("User-Agent","")),"logout")
...
<div class="glass wide admin-section"><h3 class="title">😀 Emoji Status</h3><div class="emoji-grid">'''
        ...
        if path == "/api/version":
            self.send_json({"current":VERSION,"latest":get_remote_version()}); return
...
<article class="glass card backup-health-card"><div class="card-head"><div style="display:flex;gap:12px">{ui_icon("health", "card-icon")}<div><h3 class="title">Backup Health</h3><p class="sub">سلامت بکاپ و سرویس‌ها</p></div></div><span class="health-neon">✦</span></div><div class="health-list">{''.join(f'<div class="meta-row has-status"><span class="meta-label">{status_dot("ok" if ok else "bad")} {html.escape(name)}</span><strong>{html.escape(detail)}</strong></div>' for name,ok,detail in get_health_info(c,panel_info))}</div></article>
...
<article class="glass wide"><div class="card-head"><div style="display:flex;gap:12px">{ui_icon("chart", "card-icon")}<div><h3 class="title">منابع سرور مجازی</h3><p class="sub">CPU · RAM · Disk · پایش لحظه‌ای</p></div></div></div>{_resource_chart_html()}</article>
            ...
            body += '<article class="glass wide"><div class="card-head"><div><h3 class="title">⚡ پالت فرمان</h3><p class="sub">تشخیص خطا · امنیت · اعلان‌ها · نشست‌ها · خط زمانی</p></div></div><div class="actions"><a class="btn" href="/diagnostics">{ui_icon("health", "inline-icon")} تشخیص خطا</a><a class="btn" href="/security-center">{ui_icon("shield", "inline-icon")} مرکز امنیت</a><a class="btn" href="/notifications">{ui_icon("activity", "inline-icon")} مرکز اعلان‌ها</a><a class="btn" href="/sessions">{ui_icon("account", "inline-icon")} مدیریت نشست‌ها</a><a class="btn" href="/backup-timeline">{ui_icon("clock", "inline-icon")} خط زمانی بکاپ</a><button class="btn" type="button" onclick="document.getElementById(\'cmdPalette\').classList.remove(\'hidden\');document.getElementById(\'cmdInput\').focus()">⌘ پالت فرمان</button></div></article><div id="cmdPalette" class="cmd-palette hidden"><div class="cmd-card"><input id="cmdInput" placeholder="جستجوی فرمان‌ها…" oninput="filterCmd(this.value)"><a href="/diagnostics">{ui_icon("health", "inline-icon")} تشخیص خطا</a><a href="/security-center">{ui_icon("shield", "inline-icon")} مرکز امنیت</a><a href="/notifications">{ui_icon("activity", "inline-icon")} مرکز اعلان‌ها</a><a href="/sessions">{ui_icon("account", "inline-icon")} مدیریت نشست‌ها</a><a href="/backup-timeline">{ui_icon("clock", "inline-icon")} خط زمانی بکاپ</a><button class="btn" onclick="this.closest(\'.cmd-palette\').classList.add(\'hidden\')">Esc</button></div></div><style>.cmd-palette{position:fixed;inset:0;z-index:9999;display:grid;place-items:start center;padding-top:12vh;background:rgba(3,5,12,.55);backdrop-filter:blur(12px)}.cmd-palette.hidden{display:none}.cmd-card{width:min(680px,calc(100% - 28px));padding:16px;border:1px solid var(--line);border-radius:22px;background:rgba(10,14,30,.9);box-shadow:0 30px 100px rgba(0,0,0,.55)}.cmd-card input{width:100%;padding:14px;border-radius:14px;border:1px solid var(--line);background:rgba(255,255,255,.05);color:var(--text);margin-bottom:10px}.cmd-card a{display:block;padding:11px;border-radius:12px;color:var(--text);text-decoration:none}.cmd-card a:hover{background:rgba(255,255,255,.07)}</style><script>function filterCmd(q){document.querySelectorAll(\'.cmd-card a\').forEach(a=>a.style.display=a.textContent.toLowerCase().includes(q.toLowerCase())?\'block\':\'none\')}document.addEventListener(\'keydown\',e=>{if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()===\'k\'){e.preventDefault();document.getElementById(\'cmdPalette\')?.classList.remove(\'hidden\');document.getElementById(\'cmdInput\')?.focus()}if(e.key===\'Escape\')document.getElementById(\'cmdPalette\')?.classList.add(\'hidden\')})</script>'
            for _ph, _ic in {
                '{ui_icon("health", "inline-icon")}': ui_icon("health", "inline-icon"),
                '{ui_icon("shield", "inline-icon")}': ui_icon("shield", "inline-icon"),
                '{ui_icon("activity", "inline-icon")}': ui_icon("activity", "inline-icon"),
                '{ui_icon("account", "inline-icon")}': ui_icon("account", "inline-icon"),
                '{ui_icon("clock", "inline-icon")}': ui_icon("clock", "inline-icon"),
            }.items():
                body = body.replace(_ph, _ic)
        ...
        if path in {"/diagnostics","/security-center","/notifications","/sessions","/backup-timeline"}:
            if path=="/diagnostics": body='<section class="hero"><h2>'+ui_icon("health","hero-icon")+' <span class="gradient">تشخیص خطا</span></h2><p>بررسی سریع سرویس‌ها، Backup، Telegram، Scheduler و منابع سیستم.</p></section><div class="glass wide">'+''.join(f'<div class="meta-row"><span>{html.escape(n)}</span><strong>{html.escape(d)}</strong></div>' for n,ok,d in get_health_info(c,get_panel_info()))+'</div>'
            elif path=="/security-center": body='<section class="hero"><h2>'+ui_icon("shield","hero-icon")+' <span class="gradient">مرکز امنیت</span></h2><p>وضعیت کنترل‌های امنیتی و نشست‌های مسدودشده.</p></section><div class="glass wide"><div class="meta-row"><span>CSRF</span><strong>فعال</strong></div><div class="meta-row"><span>Session TTL</span><strong>12 ساعت</strong></div><div class="meta-row"><span>Admin Session TTL</span><strong>2 ساعت</strong></div><div class="meta-row"><span>IP / Device مسدود</span><strong>'+str(len(_load_blocked_sessions()))+'</strong></div></div>'
            elif path=="/notifications": notifs=get_audit_logs()[:12]; body='<section class="hero"><h2>'+ui_icon("activity","hero-icon")+' <span class="gradient">مرکز اعلان‌ها</span></h2><p>آخرین رویدادهای مهم سیستم و Backup.</p></section><div class="glass wide">'+(''.join(f'<div class="meta-row"><span>{html.escape(str(x.get("message","رویداد")))}</span><strong>{html.escape(time.strftime("%Y-%m-%d %H:%M:%S",time.localtime(float(x.get("time",0) or 0))))}</strong></div>' for x in notifs) or '<div class="empty">اعلانی وجود ندارد.</div>')+'</div>'
            elif path=="/sessions":
                session_rows=[]
                current=self.sid()
                for sid,v in list(SESSIONS.items()):
                    ip=str(v.get("ip") or "unknown"); dev=str(v.get("device") or "Unknown"); age=_relative_time(v.get("created",time.time()))
                    actions=f'<form method="post" action="/session-kick">{hidden_csrf(self.sid())}<input type="hidden" name="sid" value="{html.escape(sid,quote=True)}"><button class="btn" type="submit">{ui_icon("activity","inline-icon")} خروج</button></form>'
                    if sid != current:
                        actions += f'<form method="post" action="/session-block">{hidden_csrf(self.sid())}<input type="hidden" name="sid" value="{html.escape(sid,quote=True)}"><button class="btn danger" type="submit">{ui_icon("shield","inline-icon")} بلاک IP + خروج</button></form>'
                    else:
                        actions += '<span class="status on">نشست فعلی</span>'
                    session_rows.append(f'<div class="session-row"><div><strong>{html.escape(str(v.get("username","user")))}</strong><div class="sub">{html.escape(ip)} · {html.escape(dev)} · {html.escape(age)}</div></div><div class="actions compact">{actions}</div></div>')
                blocked=_load_blocked_sessions()
                blocked_rows=''.join(f'<div class="meta-row"><span>{html.escape(str(x.get("ip","unknown")))} · {html.escape(str(x.get("device","Unknown")))}</span><strong>{html.escape(_relative_time(x.get("time",0)))}</strong></div>' for x in blocked[-20:][::-1]) or '<div class="empty">لیست بلاک خالی است.</div>'
                body='<section class="hero"><h2>'+ui_icon("shield","hero-icon")+' <span class="gradient">مدیریت نشست‌ها</span></h2><p>نشست مشکوک را بررسی کنید؛ با یک کلیک IP و دستگاه را بلاک و نشست را خارج کنید.</p></section><div class="glass wide"><div class="session-list">'+(''.join(session_rows) or '<div class="empty">نشست فعالی وجود ندارد.</div>')+'</div><div class="section-label">IP / Device های بلاک‌شده</div>'+blocked_rows+'</div>' 
            else: body='<section class="hero"><h2>🕒 Backup Timeline</h2><p>فقط ۵ Backup اخیر.</p></section><div class="glass wide">'+''.join(f'<div class="meta-row"><span>📦 {html.escape(str(x.get("name","Backup")))}</span><strong>{html.escape(str(x.get("time") or x.get("created_at") or "—"))}</strong></div>' for x in _load_backup_history()[-5:][::-1])+'</div>'
            self.send_html(page(path.strip('/').replace('-',' ').title(),body)); return
  ...
  <div class="latest-backup-actions"><a class="btn primary" href="/backup-download?name={q}">{ui_icon("download", "inline-icon")} دانلود مستقیم Backup</a><form method="post" action="/backup-send-telegram">{hidden_csrf(self.sid())}<input type="hidden" name="name" value="{name}"><button class="btn" type="submit">{ui_icon("telegram", "inline-icon")} ارسال مجدد به Telegram</button></form><form method="post" action="/backup-delete">{hidden_csrf(self.sid())}<input type="hidden" name="name" value="{name}"><button class="btn danger" type="submit">{ui_icon("trash", "inline-icon")} حذف Backup</button></form></div>
        ...
        if path == "/logs":
            ...
            rows=[]
            labels = {
                "login_success":"🔐 ورود موفق به پنل", "login_failed":"🚫 تلاش ورود ناموفق",
                "admin_login_success":"👑 ورود موفق بخش مدیریت", "admin_login_failed":"👑 ورود ناموفق بخش مدیریت",
                "backup_start":"📦 شروع Backup", "backup_created":"✅ ساخت موفق Backup",
                "backup_failed":"❌ خطای Backup", "telegram_send_success":"📨 ارسال موفق Backup به Telegram",
                "telegram_send_failed":"⚠️ شکست ارسال Backup به Telegram", "telegram_test_success":"🧪 تست موفق Telegram",
                "telegram_test_failed":"❌ تست ناموفق Telegram", "scheduler_start":"⏱️ شروع Scheduler",
                "scheduler_restart":"⏱️ Restart Scheduler", "scheduler_stop":"⏱️ Stop Scheduler",
                "telegram_settings":"⚙️ تغییر تنظیمات Telegram", "account_settings":"👤 تغییر تنظیمات حساب",
                "backup_delete":"🗑️ حذف Backup", "logout":"↩️ خروج از پنل", "language_change":"🌐 تغییر زبان",
            }
            for x in logs[:200]:
                ts=float(x.get("time",0) or 0)
                label=labels.get(str(x.get("event","activity")), x.get("message","Activity"))
                meta=f'{x.get("ip","unknown")} · {x.get("device","Unknown")}'
                rows.append(f'<div class="meta-row"><span>{html.escape(str(label))}</span><strong>{html.escape(meta)}<br><small>{html.escape(time.strftime("%Y-%m-%d %H:%M:%S",time.localtime(ts)))}</small></strong></div>')
    ...
    def do_POST(self):
        ...
        if path == "/login":
            ...
            if username_ok and check_password(data.get("password", ""), c):
                ...
                SESSIONS[sid] = {"created": time.time(), "csrf": secrets.token_urlsafe(24), "role": "user", "username": canonical_username(c.get("username","admin")), "ip": login_ip, "device": _login_device(self.headers.get("User-Agent","")), "login_notice":{"text":"ورود با موفقیت انجام شد","ip":login_ip,"device":_login_device(self.headers.get("User-Agent",""))}}
                self.send_response(302)
        ...
        if path == "/account":
            ...
            if pw or confirm:
                if not valid_password(pw):
                    self.send_html(page("Account", '<div class="glass"><div class="notice bad">رمز جدید باید حداقل ۸ کاراکتر، شامل حروف انگلیسی، یک عدد و یک کاراکتر ویژه باشد.</div><a class="btn" href="/account">تلاش دوباره</a></div>'), 400); return
                if pw != confirm:
        ...
        if path == "/telegram":
            ...
            if c["telegram_auto_delete"] and c["telegram_auto_delete_hours"]<0.5: c["telegram_auto_delete_hours"]=1.0
        ...
        if path == "/backup-send-telegram":
            ...
            try:
                core=load_core(); now_str=time.strftime("%Y-%m-%d %H:%M:%S")
                caption=f"PasarGuard + PG-Node Auto Backup\nDate: {now_str}\nInterval: {c.get('interval','24')}h\ndurwinam"
                ok,msg=send_archive(core,str(target),c,caption)
                _record_activity("Backup resent to Telegram" if ok else "Backup Telegram resend failed","ok" if ok else "bad",canonical_username(c.get("username","admin")),self.client_address[0] if self.client_address else "unknown",_login_device(self.headers.get("User-Agent","")),"telegram_resend_success" if ok else "telegram_resend_failed")
                self.send_html(page("Backup Telegram",f'<div class="glass"><div class="notice {"ok" if ok else "bad"}">{status_dot("ok" if ok else "bad")} {"Backup دوباره به Telegram ارسال شد." if ok else "ارسال مجدد Backup ناموفق بود."}<br><span class="empty">{html.escape(str(msg))}</span></div><a class="btn" href="/backups">بازگشت</a></div>')); return
            except Exception as exc:
                self.send_html(page("Backup Telegram",f'<div class="glass"><div class="notice bad">ارسال مجدد ناموفق بود: {html.escape(str(exc))}</div><a class="btn" href="/backups">بازگشت</a></div>'),500); return

        if path == "/session-kick":
            sid_target=data.get("sid","")
            if sid_target in SESSIONS:
                info=SESSIONS.pop(sid_target)
                _record_activity("Session kicked","warn",canonical_username(c.get("username","admin")),self.client_address[0] if self.client_address else "unknown",_login_device(self.headers.get("User-Agent","")),"session_kick")
            self.redirect("/sessions"); return

        if path == "/session-block":
            sid_target=data.get("sid","")
            info=SESSIONS.get(sid_target)
            if info:
                _block_session_target(info.get("ip"),info.get("device"),info.get("username"))
                _record_activity(f"Blocked session {info.get('ip','unknown')}","warn",canonical_username(c.get("username","admin")),self.client_address[0] if self.client_address else "unknown",_login_device(self.headers.get("User-Agent","")),"session_block")
            self.redirect("/sessions"); return
...
def cleanup_telegram_messages():
    ...
    jobs=list(cfg.get("telegram_delete_jobs") or [])
    if not jobs: return
    # Five newest Telegram backup messages are always protected.
    protected={id(x) for x in sorted(jobs,key=lambda x:float(x.get("sent_at",0)),reverse=True)[:5]}
    keep=[]
    for job in jobs:
        if id(job) in protected or float(job.get("delete_at",0))>time.time():
            keep.append(job); continue
        try:
            ok,_=telegram_request(cfg.get("token"),"deleteMessage",{"chat_id":job.get("chat_id"),"message_id":job.get("message_id")},cfg.get("proxy") or None,30)
            if not ok: pass
        except Exception: pass
    if len(keep)!=len(jobs): cfg["telegram_delete_jobs"]=keep[-500:]; save_cfg(cfg)

def worker():
    # Scheduler loop also services Telegram auto-delete every minute.
    last_backup=0.0
    while True:
        ...
        if c.get("token") and c.get("chat") and (time.time()-last_backup)>=interval:
            try:
                core=load_core(); archive=core.create_backup(bool(c.get("node",False)))
                if archive and os.path.exists(archive):
                    _record_backup_created(archive)
                ...
                print("[-] Scheduled backup failed:",e,flush=True); last_backup=time.time()
        time.sleep(60)
